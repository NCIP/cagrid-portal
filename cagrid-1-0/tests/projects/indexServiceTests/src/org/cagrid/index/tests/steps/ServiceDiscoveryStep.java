...
public class ServiceDiscoveryStep extends Step {
    ...
    public ServiceDiscoveryStep(EndpointReferenceType indexServiceEndpoint,
        EndpointReferenceType domainServiceEndpoint, StringBuffer expectedMetadata, boolean shouldBeFound) {
        super();

        this.shouldBeFound = shouldBeFound;
        this.indexServiceEndpoint = indexServiceEndpoint;
        StringReader reader = new StringReader(expectedMetadata.toString());
        try {
            this.metadata = (ServiceMetadata) Utils.deserializeObject(reader, ServiceMetadata.class);
        } catch (Exception ex) {
            ex.printStackTrace();
            fail("Error deserializing expectedf service metadata: " + ex.getMessage());
        }
