...
public class GrouperGrantPrivilegeStep
	extends Step
{
	...
	public GrouperGrantPrivilegeStep(String path, String subject, String priv)
	{
		...
		throws GridGrouperRuntimeFault, StemNotFoundFault, RemoteException, MalformedURIException
	{
		...
		// grant
		if (isGroup) {
			grouper.grantGroupPrivilege(new GroupIdentifier(null, path), subject, GroupPrivilegeType.fromString(priv));
		} else {
			grouper.grantStemPrivilege(new StemIdentifier(null, path), subject, StemPrivilegeType.fromString(priv));
		}
	}
