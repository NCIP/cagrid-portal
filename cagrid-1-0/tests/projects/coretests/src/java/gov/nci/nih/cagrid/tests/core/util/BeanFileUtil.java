...
public class BeanFileUtil
{
	...
	public void writeBean(int depth, String qname, File outFile) 
		throws Exception
	{
	...
	public Object readBean(File inFile) 
		throws Exception
	{
		Class cl = IntroduceServiceInfo.loadClass(serviceDir, className);;
	...
	public static Options getOptions()
	{
		...
		Option dir = OptionBuilder.withArgName("dir")
			.hasArg()
			.isRequired(true)
			.withDescription("the service dir to look for beans")
			.create("dir");

		Option out = OptionBuilder.withArgName("out")
			.hasArg()
			.isRequired(true)
			.withDescription("the output xml file")
			.create("out");

		Option cl = OptionBuilder.withArgName("class")
			.hasArg()
			.isRequired(true)
			.withDescription("the name of the root class to generate an object for")
			.create("class");
		
		Option qname = OptionBuilder.withArgName("qname")
			.hasArg()
			.isRequired(true)
			.withDescription("the qname for the root element")
			.create("qname");

		Option depth = OptionBuilder.withArgName("depth")
			.hasArg()
			.isRequired(false)
			.withDescription("depth of classes to create")
			.create("depth");
    	...
    	throws Exception
    {
		...
		int depth = 0;
		if (cmd.hasOption("depth")) depth = Integer.parseInt(cmd.getOptionValue("depth"));
		
		BeanFileUtil bean = new BeanFileUtil(new File(cmd.getOptionValue("dir")), cmd.getOptionValue("class"));
		bean.writeBean(
			depth, 
			cmd.getOptionValue("qname"),
