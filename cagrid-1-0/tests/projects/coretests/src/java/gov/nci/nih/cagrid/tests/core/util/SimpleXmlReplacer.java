...
public class SimpleXmlReplacer
{
	...
	public void performReplacement(File inFile) 
		throws IOException
	{
		performReplacement(inFile, inFile);
	}
	
	public void performReplacement(File inFile, File outFile) 
		throws IOException
	{
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (index != -1) {
					...
					if (index2 == -1) index2 = line.indexOf("/>", index);
					if (index2 == -1) index2 = line.indexOf('>', index);
					if (index2 != -1) {
						...
						if (value != null) {
							...
							if (index != -1) {
								line = line.substring(0, index) + ">" + value + "</" + name + ">" + line.substring(index+2);
