	...
	public static void main(String[] args) {
		...
		try {
			...
			if (line.hasOption(HELP_OPT)) {
				...
				formatter.printHelp(ConvertCAFileNameToHash.class.getName(), options);
				System.exit(0);
			} else {
				try{
					...
					File hashFile = new File(dir.getAbsolutePath()+File.separator+hash+".0");
					CertUtil.writeCertificate(cert, hashFile);
					System.out.println("Wrote hash certificate to: "+hashFile.getAbsolutePath());
					File signingPolicy = new File(dir.getAbsolutePath()+File.separator+hash+".signing_policy");
					CertUtil.writeSigningPolicy(cert, signingPolicy);
					System.out.println("Wrote hash signing policy to: "+signingPolicy.getAbsolutePath());
				}catch (Exception ex) {
					ex.printStackTrace();
				}
			}
