...
public class TestStems extends TestCase {
	...
	public void testRootStem() {
		try {
			...
			checkStem(grouper.getStem(
					AnonymousGridUserSubject.ANONYMOUS_GRID_USER_ID, Utils
							.getRootStemIdentifier()), displayExtension,
					description);
			...
			grouper.updateStem(ADMIN_USER, Utils.getStemIdentifier(root),
					update1);
			...
			grouper.grantStemPrivilege(ADMIN_USER, Utils
			...
			String[] subs1 = grouper.getSubjectsWithStemPrivilege(ADMIN_USER,
					Utils.getStemIdentifier(root), StemPrivilegeType.stem);
			assertNotNull(subs1);
			...
			String[] subs2 = grouper.getSubjectsWithStemPrivilege(ADMIN_USER,
					Utils.getStemIdentifier(root), StemPrivilegeType.create);
			assertNotNull(subs2);
			...
			grouper.revokeStemPrivilege(USER_A, Utils.getStemIdentifier(root),
	...
	public void testAddingGroups() {
		try {
			...
			assertTrue(grouper.hasStemPrivilege(SUPER_USER, Utils
					.getStemIdentifier(test), ADMIN_USER,
					StemPrivilegeType.stem));
			assertTrue(grouper.hasStemPrivilege(SUPER_USER, Utils
					.getStemIdentifier(test), ADMIN_USER,
			...
			assertEquals(0, grouper.getChildGroups(ADMIN_USER, Utils
					.getStemIdentifier(test)).length);
			grouper.addChildGroup(ADMIN_USER, Utils.getStemIdentifier(test),
					group1Extension, group1DisplayExtension);
			GroupDescriptor[] grps = grouper.getChildGroups(ADMIN_USER, Utils
			...
			assertFalse(grouper.hasStemPrivilege(SUPER_USER, Utils
					.getStemIdentifier(test), USER_A, StemPrivilegeType.stem));
			assertFalse(grouper.hasStemPrivilege(SUPER_USER, Utils
					.getStemIdentifier(test), USER_A, StemPrivilegeType.create));

			try {
				grouper.addChildGroup(USER_A, Utils.getStemIdentifier(test),
			...
			grouper.grantStemPrivilege(SUPER_USER, Utils
					.getStemIdentifier(test), USER_A, StemPrivilegeType.create);
			...
			grouper.addChildGroup(USER_A, Utils.getStemIdentifier(test),
					group2Extension, group2DisplayExtension);
			grps = grouper
					.getChildGroups(USER_A, Utils.getStemIdentifier(test));
	...
	public void testChildStems() {
		try {
			...
			try {
				grouper.addChildStem(USER_A, Utils.getStemIdentifier(root),
			...
			StemDescriptor childX = grouper.addChildStem(USER_A, Utils
			...
			grouper.grantStemPrivilege(USER_A, Utils.getStemIdentifier(childX),
			...
			StemDescriptor childX1 = grouper.addChildStem(USER_A, Utils
			...
			grouper.grantStemPrivilege(USER_A,
					Utils.getStemIdentifier(childX1), USER_A,
					StemPrivilegeType.create);
			assertTrue(grouper.hasStemPrivilege(
					AnonymousGridUserSubject.ANONYMOUS_GRID_USER_ID, Utils
							.getStemIdentifier(childX1), USER_A,
