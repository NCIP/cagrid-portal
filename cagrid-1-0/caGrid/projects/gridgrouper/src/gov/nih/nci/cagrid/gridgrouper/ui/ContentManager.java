...
public class ContentManager extends JTabbedPane {
	...
	public void addStem(StemTreeNode node) {
		...
		this.addTab(node.getStem().getDisplayExtension(), new CombinedIcon(new ContentManagerTabCloseIcon(),
			GridGrouperLookAndFeel.getStemIcon16x16()), browser, null);
	...
	public void addGroup(GroupTreeNode node) {
		...
		this.addTab(node.getGroup().getDisplayExtension(), new CombinedIcon(new ContentManagerTabCloseIcon(),
			GridGrouperLookAndFeel.getGroupIcon16x16()), browser, null);
		this.remove(getWelcomePanel());
		this.setSelectedComponent(browser);
	}
	...
	public void removeAllNodes(String gridGrouper) {
		...
		while (stemItr.hasNext()) {
			...
			if (gridGrouper.equals(sb.getStemNode().getStem().getGridGrouper().getName())) {
		...
		while (groupItr.hasNext()) {
			...
			if (gridGrouper.equals(sb.getGroupNode().getGroup().getGridGrouper().getName())) {
				nodesToRemove.add(sb.getGroupNode());
			}
		}

		for (int i = 0; i < nodesToRemove.size(); i++) {
			GridGrouperBaseTreeNode node = (GridGrouperBaseTreeNode) nodesToRemove.get(i);
