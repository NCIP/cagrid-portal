        ...
        try {
            ...
            _logger.debug("Getting ID for service:" + epr + ".");
             //List resultSet =  getHibernateTemplate().find("Select index.pk from IndexService index where index.epr = ?", epr);
               List resultSet = getSession().createSQLQuery("Select id_key from INDEX_SERVICE where service_epr = '" + epr + "';").list();

            /** if epr is not index then try
             * the services table
             */
            if(resultSet.isEmpty()){
                _logger.debug("EPR not found in index service. Trying Registered Services" + epr + ".");
