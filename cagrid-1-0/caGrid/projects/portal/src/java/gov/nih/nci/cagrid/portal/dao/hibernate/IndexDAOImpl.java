...
public class IndexDAOImpl extends GridServiceBaseDAOImpl implements IndexDAO {
     ...
     * @param rService
     */
    public IndexService addRegisteredService(IndexService idx, RegisteredService rService) {
        ...
        try {
            ...
            logger.debug("No record found for service " + rService.getEpr() + ". Hibernate will assign ID");
        } catch (Exception e){
            logger.error("Unable to retreive ID for service " + rService.getEpr() + ". Nested Exception is" + e.getMessage());
        }
