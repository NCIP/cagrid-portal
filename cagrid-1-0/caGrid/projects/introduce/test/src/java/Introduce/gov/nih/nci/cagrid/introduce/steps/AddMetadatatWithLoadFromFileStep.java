...
public class AddMetadatatWithLoadFromFileStep extends Step {
	...
	public void runStep() throws Throwable {
		...
		metadata.setLocation("../cagrid/types/common/CommonServiceMetadata.xsd");
		metadata
		...
		if (metadatasType.getMetadata() != null) {
			...
			System.arraycopy(metadatasType.getMetadata(), 0, newMetadatas, 0,
					metadatasType.getMetadata().length);
		} else {
			newLength = 1;
			newMetadatas = new MetadataType[newLength];
		}
		...
		String serviceInterface = pathtobasedir + File.separator + tci.dir
				...
				+ tci.getPackageDir() + "/service/" 
				+ File.separator + "globus"
				+ File.separator + "resource"
				+ File.separator + "BaseResource.java";
		assertTrue("Checking that BaseResource contains the load method",StepTools.methodExists(serviceInterface,"loadCommonServiceMetadataFromFile"));
