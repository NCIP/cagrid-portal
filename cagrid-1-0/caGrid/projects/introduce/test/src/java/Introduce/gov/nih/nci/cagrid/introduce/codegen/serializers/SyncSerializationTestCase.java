...
public class SyncSerializationTestCase extends TestCase {
	...
	private static final String NO_MARKUP = IGNORED + SyncSerialization.WSDD_END_TAG;

	private static final String ONLY_HEADER_MARKUP = IGNORED + SyncSerialization.MAPPING_HEADER
		+ SyncSerialization.WSDD_END_TAG;

	private static final String ALL_MARKUP = IGNORED + SyncSerialization.MAPPING_HEADER + "\n" + EXISTING + "\n"
		+ SyncSerialization.MAPPING_FOOTER + SyncSerialization.WSDD_END_TAG;

	private static final String GOLD = IGNORED + SyncSerialization.MAPPING_HEADER + "\n" + REPLACEMENT + "\n"
		+ SyncSerialization.MAPPING_FOOTER + SyncSerialization.WSDD_END_TAG + "\n";

	private static final String NO_REPLACEMENT_GOLD = IGNORED + SyncSerialization.WSDD_END_TAG + "\n";
	...
	protected void setUp() throws Exception {
		...
		onlyHeaderFile = File.createTempFile(this.getClass().getName(), ".wsdd");
		onlyHeaderFile.deleteOnExit();
		fw = new FileWriter(onlyHeaderFile);
		fw.write(ONLY_HEADER_MARKUP);
		fw.close();

		fullMarkupFile = File.createTempFile(this.getClass().getName(), ".wsdd");
