...
public class GlobusHelper {
	...
	public void createTempGlobus() throws IOException {
		...
		FileUtils.copyRecursive(new File(globusLocation), tmpGlobusLocation,
				null);
	}

	public void deployService(File serviceDir) throws IOException,
	...
	public void deployService(File serviceDir, String target)
		...
		String[] envp = new String[] { "GLOBUS_LOCATION="
				+ tmpGlobusLocation.getAbsolutePath(), };
	...
	public void startGlobus() throws IOException {
		p = runGlobusCommand("org.globus.wsrf.container.ServiceContainer");
	...
	private Process runGlobusCommand(String clName) throws IOException {
		...
		File java = new File(System.getProperty("java.home"), "bin"
				+ File.separator + "java");
		File lib = new File(tmpGlobusLocation, "lib");
		String classpath = lib.getAbsolutePath() + File.separator
				+ "bootstrap.jar";
		classpath += File.pathSeparator + lib.getAbsolutePath()
				+ File.separator + "cog-url.jar";
		classpath += File.pathSeparator + lib.getAbsolutePath()
				+ File.separator + "axis-url.jar";
		...
		Process p = Runtime.getRuntime().exec(
				(String[]) cmd.toArray(new String[0]), envp,
	...
	public boolean isGlobusRunning() {
		try {
			...
			EngineConfiguration engineConfig = new FileProvider(System
					.getenv("GLOBUS_LOCATION")
					+ File.separator + "client-config.wsdd");
			locator.setEngine(new AxisClient(engineConfig));

			CounterPortType counter = locator
					.getCounterPortTypePort(new EndpointReferenceType(
							new Address("http://localhost:" + port
									+ "/wsrf/services/CounterService")));
