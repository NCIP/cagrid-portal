 ...
 *          Exp $
 */
public class CADSRParameterConfigurationComponent extends GridPortalComponent {
	...
	public CADSRParameterConfigurationComponent(Vector typeInfo, File schemaDir, boolean handleParameterName) {
	...
	private CADSRConfigurationPanel getCADSRPanel() {
		if (cadsrPanel == null) {
			cadsrPanel = new CADSRConfigurationPanel(CADSRConfigurationPanel.TYPES_ONLY);
			cadsrPanel.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "",
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					try {
						XMLDataModelService handle = (XMLDataModelService) GridServiceResolver.getInstance()
							.getGridService("http://dc01.bmi.ohio-state.edu:8080/wsrf/services/cagrid/GlobalModelExchange");
						if (cadsrPanel.getCADSRClassDescriptor()!= null) {
							CADSRClassDescriptor descriptor = cadsrPanel.getCADSRClassDescriptor();
							Namespace namespace = new Namespace("gme://" + descriptor.getClassificationScheme() + "." + descriptor.getContext() + "/" + descriptor.getVersion() + "/" + descriptor.getSchemeItem());
							handle.cacheSchema(namespace, schemaDir);
						}
					} catch (MobiusException e1) {
						e1.printStackTrace();
						JOptionPane.showMessageDialog(me,
							"Please check the  URL and make sure that you have the appropriate credentials!");
					}
