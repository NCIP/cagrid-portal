...
public class ExtensionsManagerPanel extends JPanel {
		...
		if (info.getExtensions() != null
			...
			for (int i = info.getExtensions().getExtension().length - 1; i >= 0; i--) {
				...
				ServiceExtensionDescriptionType desc = ExtensionsLoader
						.getInstance().getServiceExtension(ext.getName());
				if (desc != null) {
	...
	private ExtensionsTable getExtensionsTable() {
		if (extensionsTable == null) {
			...
			extensionsTable.getSelectionModel().addListSelectionListener(
					new ListSelectionListener() {
						...
						public void valueChanged(ListSelectionEvent e) {
							...
							if (row < 0 || row >= extensionsTable.getRowCount()) {
								...
								try {
									ServiceExtensionDescriptionType etype = ExtensionsLoader
											.getInstance()
											.getServiceExtensionByDisplayName(
													extensionsTable
															.getSelectedRowData());
									if (etype.getServiceExtensionRemover() != null) {
										getRemoveExtensionButton().setEnabled(
												true);
									} else {
										getRemoveExtensionButton().setEnabled(
												false);
