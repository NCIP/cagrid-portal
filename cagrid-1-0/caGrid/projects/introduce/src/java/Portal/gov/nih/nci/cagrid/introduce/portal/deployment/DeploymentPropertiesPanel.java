...
public class DeploymentPropertiesPanel extends JPanel {
	...
	private void initialize() {
		...
		this.setBorder(BorderFactory.createTitledBorder(null,
				"Deployment Properties", TitledBorder.DEFAULT_JUSTIFICATION,
	...
	private void validateInput() {
		...
		if (!ValidationUtils.isNotBlank(getDeploymentPrefixTextField()
				.getText())) {
			result.add(new SimpleValidationMessage(
					"Deployment prefix must not be blank.", Severity.ERROR,
					IntroduceConstants.INTRODUCE_DEPLOYMENT_PREFIX_PROPERTY));
		} else if (!ValidationUtils
				.isAlphanumeric(getDeploymentPrefixTextField().getText())) {
			result.add(new SimpleValidationMessage(
					"Deployment prefix must be alpha numeric.", Severity.ERROR,
					IntroduceConstants.INTRODUCE_DEPLOYMENT_PREFIX_PROPERTY));
		}

		if (!ValidationUtils
				.isNotBlank(getIndexServiceURLTextField().getText())) {
			result
					.add(new SimpleValidationMessage(
							"Index service must not be blank.",
							Severity.ERROR,
							IntroduceConstants.INTRODUCE_DEPLOYMENT_INDEX_SERVICE_URL_PROPERTY));
		}

		if (!ValidationUtils.isNotBlank(getIndexServiceRefreshjTextField()
				.getText())) {
			result
					.add(new SimpleValidationMessage(
							"Index service refresh must not be blank.",
							Severity.ERROR,
							IntroduceConstants.INTRODUCE_DEPLOYMENT_INDEX_REFRESH_PROPERTY));
		} else if (!ValidationUtils
				.isNumeric(getIndexServiceRefreshjTextField().getText())) {
			result
					.add(new SimpleValidationMessage(
							"Index service refresh must be numeric",
							Severity.ERROR,
							IntroduceConstants.INTRODUCE_DEPLOYMENT_INDEX_REFRESH_PROPERTY));
		}

		if (!ValidationUtils.isNotBlank(getRegistrationRefreshjTextField()
				.getText())) {
			result
					.add(new SimpleValidationMessage(
							"Registration refresh must not be blank.",
							Severity.ERROR,
							IntroduceConstants.INTRODUCE_DEPLOYMENT_REFRESH_REGISTRATION_PROPERTY));
		} else if (!ValidationUtils
				.isNumeric(getRegistrationRefreshjTextField().getText())) {
			result
					.add(new SimpleValidationMessage(
							"Registration refresh must be numeric",
							Severity.ERROR,
							IntroduceConstants.INTRODUCE_DEPLOYMENT_REFRESH_REGISTRATION_PROPERTY));
		}
	...
	private JPanel getContentPanel() {
		if (contentPanel == null) {
			...
			refreshRegistationLabel.setFont(refreshRegistationLabel.getFont()
			...
			indexRefreshLabel.setFont(indexRefreshLabel.getFont().deriveFont(
					Font.BOLD));
			...
			indexServiceURLLabel.setFont(indexServiceURLLabel.getFont()
					.deriveFont(Font.BOLD));
			...
			deploymentPrefixLabel.setFont(deploymentPrefixLabel.getFont()
					.deriveFont(Font.BOLD));
			...
			registrationLabel.setFont(registrationLabel.getFont().deriveFont(
					Font.BOLD));
			...
			contentPanel.add(getDeploymentPrefixTextField(),
					gridBagConstraints6);
			contentPanel.add(getPerformRegistrationCheckBox(),
					gridBagConstraints7);
			contentPanel
					.add(getIndexServiceURLTextField(), gridBagConstraints8);
			contentPanel.add(getIndexServiceRefreshjTextField(),
					gridBagConstraints11);
			contentPanel.add(getRegistrationRefreshjTextField(),
					gridBagConstraints12);
			contentPanel.add(getIndexIntervalSelectionComboBox(),
					gridBagConstraints9);
			contentPanel.add(getRegistrationIntervalSelectionComboBox(),
	...
	private JTextField getDeploymentPrefixTextField() {
		if (deploymentPrefixTextField == null) {
			...
			deploymentPrefixTextField
					.setText(deploymentProperties
							.getProperty(IntroduceConstants.INTRODUCE_DEPLOYMENT_PREFIX_PROPERTY));
			deploymentPrefixTextField.getDocument().addDocumentListener(
	...
	private JTextField getIndexServiceURLTextField() {
		if (indexServiceURLTextField == null) {
			...
			indexServiceURLTextField
					.setText(deploymentProperties
							.getProperty(IntroduceConstants.INTRODUCE_DEPLOYMENT_INDEX_SERVICE_URL_PROPERTY));
			indexServiceURLTextField.getDocument().addDocumentListener(
					new DocumentListener() {
	...
	private JTextField getIndexServiceRefreshjTextField() {
		if (indexServiceRefreshjTextField == null) {
			...
			indexServiceRefreshjTextField
					.setText(String
							.valueOf(Integer
									.parseInt(deploymentProperties
											.getProperty(IntroduceConstants.INTRODUCE_DEPLOYMENT_INDEX_REFRESH_PROPERTY)) / 60000));
			indexServiceRefreshjTextField.getDocument().addDocumentListener(
					new DocumentListener() {
	...
	private JTextField getRegistrationRefreshjTextField() {
		if (registrationRefreshjTextField == null) {
			...
			registrationRefreshjTextField
					.setText(String
							.valueOf(Integer
									.parseInt(deploymentProperties
											.getProperty(IntroduceConstants.INTRODUCE_DEPLOYMENT_REFRESH_REGISTRATION_PROPERTY)) / 60));
			registrationRefreshjTextField.getDocument().addDocumentListener(
					new DocumentListener() {
	...
	public Properties getDeploymentProperties() {
		...
		props.put(IntroduceConstants.INTRODUCE_DEPLOYMENT_PREFIX_PROPERTY,
				getDeploymentPrefixTextField().getText());
		props
				.put(
						IntroduceConstants.INTRODUCE_DEPLOYMENT_PERFORM_REGISTRATION_PROPERTY,
						String.valueOf(getPerformRegistrationCheckBox()
								.isSelected()));
		props
				.put(
						IntroduceConstants.INTRODUCE_DEPLOYMENT_INDEX_SERVICE_URL_PROPERTY,
						getIndexServiceURLTextField().getText());
		int multiplier = 1;
		if (((String) getIndexIntervalSelectionComboBox().getSelectedItem())
				.equals("seconds")) {
			multiplier = 1000;
		} else if (((String) getIndexIntervalSelectionComboBox()
				.getSelectedItem()).equals("minutes")) {
			multiplier = 1000 * 60;
		} else if (((String) getIndexIntervalSelectionComboBox()
				.getSelectedItem()).equals("hours")) {
		...
		if (((String) getRegistrationIntervalSelectionComboBox()
				.getSelectedItem()).equals("minutes")) {
			multiplier = 60;
		} else if (((String) getRegistrationIntervalSelectionComboBox()
				.getSelectedItem()).equals("hours")) {
			multiplier = 60 * 60;
		} else if (((String) getRegistrationIntervalSelectionComboBox()
				.getSelectedItem()).equals("days")) {
