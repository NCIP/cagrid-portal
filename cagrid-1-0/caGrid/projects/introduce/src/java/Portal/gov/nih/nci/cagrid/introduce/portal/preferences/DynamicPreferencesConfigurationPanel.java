...
public abstract class DynamicPreferencesConfigurationPanel extends BasePreferenceConfigurationPanel {
    ...
    protected String getCheckBoxValue(String label) {
        JCheckBox field = (JCheckBox) checkBoxes.get(label);
        if (field != null) {
            return new Boolean(field.isSelected()).toString();
        } else {
            return null;
        }
    }
    ...
    protected void addCheckBox(JPanel panel, String label, boolean value, int y, boolean enabled) {
        JLabel label1 = new JLabel();
        label1.setText(label);
        JComponent component = null;
        JCheckBox text = new JCheckBox();
        text.setSelected(value);
        text.setEnabled(enabled);
        component = text;
        this.checkBoxes.put(label, text);
