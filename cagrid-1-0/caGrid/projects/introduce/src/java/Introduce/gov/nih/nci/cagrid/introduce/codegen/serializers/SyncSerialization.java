...
public class SyncSerialization extends SyncTool {
	...
	public SyncSerialization(File baseDirectory, ServiceInformation info) {
		...
		clientWSDD = new File(baseDirectory.getAbsolutePath() + File.separator + "src" 
	...
	public void sync() throws SynchronizationException {
		if (!(clientWSDD.exists() && clientWSDD.canRead() && clientWSDD.canWrite() && serverWSDD.exists()
			&& serverWSDD.canRead() && serverWSDD.canWrite())) {
			throw new SynchronizationException("Unable to locate or write to expected wsdd files: " + clientWSDD
				+ " and " + serverWSDD);
		}
