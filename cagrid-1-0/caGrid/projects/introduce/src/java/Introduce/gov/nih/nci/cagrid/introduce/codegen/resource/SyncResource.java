			...
			if (getServiceInformation().getServices() != null
				...
				for (int i = 0; i < getServiceInformation().getServices()
					...
					if (service.getResourcePropertiesList() != null
							...
							&& !service.getResourcePropertiesList()
									.getSynchronizeResourceFramework()
									.booleanValue()) {
						...
						String resourceContanstsS = resourceContanstsT
								.generate(new SpecificServiceInformation(
										getServiceInformation(), service));
						File resourceContanstsF = new File(srcDir
								.getAbsolutePath()
								+ File.separator
								+ getServiceInformation()
										.getPackageDir(service)
						...
						String metadataConfigurationS = metadataConfigurationT
								.generate(new SpecificServiceInformation(
										getServiceInformation(), service));
						File metadataConfigurationF = new File(srcDir
								.getAbsolutePath()
								+ File.separator
								+ getServiceInformation()
										.getPackageDir(service)
