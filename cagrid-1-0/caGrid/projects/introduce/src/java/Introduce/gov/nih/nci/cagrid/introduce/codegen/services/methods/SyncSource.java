...
public class SyncSource {
	...
	public static String createExceptions(MethodType method, ServiceType service) {
		String exceptions = "";
		// process the faults for this method...
		MethodTypeExceptions exceptionsEl = method.getExceptions();
		String packageName = service.getPackageName() + ".stubs";
	...
	public void addImpl(MethodType method) {
		...
		// insert the new client method
		int endOfClass = fileContent.lastIndexOf("}");
		String clientMethod = "\n\t" + createUnBoxedSignatureStringFromMethod(method, serviceInfo) + " " + createExceptions(method, service);
	...
	public void addProviderImpl(MethodType method) throws Exception {
		...
		if (method.getIsUnBoxable().booleanValue()) {
			...
			// autoboxing
			// and get appropriate
			clientMethod = "\n\t" + createBoxedSignatureStringFromMethod(method) + " " + createExceptions(method, service);
	...
	public void modifyImpl(Modification mod) throws Exception {
		...
		clientMethod = "\t" + createUnBoxedSignatureStringFromMethod(method, serviceInfo) + " " + createExceptions(method,service);
		clientMethod += "{";
