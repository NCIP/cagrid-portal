...
public class Introduce_1_0__1_1_Upgrader extends IntroduceUpgraderBase {
	...
	public Introduce_1_0__1_1_Upgrader(IntroduceUpgradeStatus status,
	...
	protected void upgrade() throws Exception {
		...
		File registrationFile = new File(getServicePath() + File.separator
				+ "etc" + File.separator + "registration.xml");
		...
		getStatus().addDescriptionLine(
				"replaced build-deploy.xml with new version");
		getStatus()
				.addIssue(
						"Replaced the build-deploy.xml file.",
		...
		File srcDir = new File(getServiceInformation().getBaseDirectory()
				.getAbsolutePath()
				+ File.separator + "src");
		for (int i = 0; i < getServiceInformation().getServices().getService().length; i++) {
			...
			if (!service.getResourceFrameworkType().equals(
				...
				if (service.getResourceFrameworkType().equals(
					...
					String baseResourceBaseS = baseResourceBaseT
							.generate(new SpecificServiceInformation(
									getServiceInformation(), service));
					File baseResourceBaseF = new File(srcDir.getAbsolutePath()
							+ File.separator
							+ CommonTools.getPackageDir(service)
							+ File.separator + "service" + File.separator
							+ "globus" + File.separator + "resource"
					...
					FileWriter baseResourceBaseFW = new FileWriter(
							baseResourceBaseF);
					...
					File baseResourceHomeF = new File(srcDir.getAbsolutePath()
							...
							+ File.separator + "service" + File.separator
							+ "globus" + File.separator + "resource"
							+ File.separator + "BaseResourceHome.java");

					FileWriter baseResourceHomeFW = new FileWriter(
							baseResourceHomeF);
					...
					String lifetimeResourceBaseS = lifetimeResourceBaseT
							.generate(new SpecificServiceInformation(
									getServiceInformation(), service));
					File lifetimeResourceBaseF = new File(srcDir
							.getAbsolutePath()
							+ File.separator
							+ CommonTools.getPackageDir(service)
							+ File.separator
					...
					FileWriter lifetimeResourceBaseFW = new FileWriter(
							lifetimeResourceBaseF);
					...
					String lifetimeResourceS = lifetimeResourceT
							.generate(new SpecificServiceInformation(
									getServiceInformation(), service));
					File lifetimeResourceF = new File(srcDir.getAbsolutePath()
							+ File.separator
							+ CommonTools.getPackageDir(service)
							+ File.separator + "service" + File.separator
							+ "globus" + File.separator + "resource"
					...
					FileWriter lifetimeResourceFW = new FileWriter(
							lifetimeResourceF);
					...
					String lifetimeResourceHomeS = lifetimeResourceHomeT
							.generate(new SpecificServiceInformation(
									getServiceInformation(), service));
					File lifetimeResourceHomeF = new File(srcDir
							.getAbsolutePath()
							+ File.separator
							+ CommonTools.getPackageDir(service)
							+ File.separator
					...
					FileWriter lifetimeResourceHomeFW = new FileWriter(
							lifetimeResourceHomeF);
				...
				getStatus().addDescriptionLine(
						"replaced resource source files for service: "
								+ service.getName());
			}
		}
		...
		getStatus().addDescriptionLine(
				"adding new deployment variables to deploy.properties");
		...
		Utils.copyFile(new File("." + File.separator + "skeleton"
				+ File.separator + "tools" + File.separator + "lib"
				+ File.separator
				+ "caGrid-1.1-Introduce-1.1-soapBindingFix.jar"), new File(
				getServicePath() + File.separator + "tools" + File.separator
						+ "lib" + File.separator
		...
		upgradeJars();
		getStatus().addDescriptionLine(
				"updating service with the new version of the jars");
	...
	private void upgradeJars() throws Exception {
		...
		FileFilter oldDkeletonLibFilter = new FileFilter() {
			public boolean accept(File name) {
				...
				boolean core = filename.startsWith("caGrid-1.0-core")
						&& filename.endsWith(".jar");
				boolean security = (filename
						.startsWith("caGrid-1.0-ServiceSecurityProvider") || filename
				...
				boolean wsrf = (filename.startsWith("globus_wsrf_mds") || filename
						...
						&& filename.endsWith(".jar");
				boolean mobius = filename.startsWith("mobius")
						&& filename.endsWith(".jar");
		...
		for (int i = 0; i < skeletonLibs.length; i++) {
			File out = new File(serviceLibDir.getAbsolutePath()
					+ File.separator + skeletonLibs[i].getName());
			try {
				Utils.copyFile(skeletonLibs[i], out);
			} catch (IOException ex) {
				throw new Exception("Error copying library (" + skeletonLibs[i]
						+ ") to service: " + ex.getMessage(), ex);
			}
			outLibs[i] = out;
		}
		// update the Eclipse .classpath file
		File classpathFile = new File(getServicePath() + File.separator
				+ ".classpath");
		try {
	...
	private void fixUpSchemaLocationsInModel() {
		...
		NamespacesType namespaces = getServiceInformation()
				.getServiceDescriptor().getNamespaces();
		if (namespaces != null && namespaces.getNamespace() != null) {
			for (NamespaceType namespace : namespaces.getNamespace()) {
				...
				if (schemaLocation != null
					...
					getStatus().addDescriptionLine(
							"Edited schema location of namespace "
									+ namespace.getNamespace());
					getStatus().addDescriptionLine(
							"\t(" + schemaLocation + " -> " + cleanLocation
									+ ")");
					editsMade = true;
				}
			}
		...
		if (editsMade) {
			getStatus()
					.addIssue(
							"Schema locations may not be platform independent",
