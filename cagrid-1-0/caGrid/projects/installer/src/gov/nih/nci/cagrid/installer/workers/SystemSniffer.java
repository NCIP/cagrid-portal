...
public class SystemSniffer {
	...
	public long getInstallationDate() {
		...
		String userHome = p.getProperty("user.home");
		try {
			String fileName = userHome + File.pathSeparator
					+ "caGridInstallation.session";
			File f = new File(fileName);
			date = f.lastModified();
		} catch (Exception ex) {
	...
	public String checkSystem() {
		...
		if (this.isFirstInstallation()) {
			...
			stbr.append("\t");
			stbr
					.append("No previous caGird installation has been found on this machine.");
			stbr.append("\n");
			stbr.append("\t");
			stbr
					.append("caGird pack will be downloaded and installed on this machine.");
			...
			if (ant_home != null) {
				...
				stbr.append("\t");
				stbr
						.append("Ant installation has not been found on this machine!");
				stbr.append("\n");
