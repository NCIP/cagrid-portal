...
public class WelcomeTextPanel extends JPanel {
	...
	public void paintComponent(Graphics g) {
		/**
		 * 
		 * super.paintComponent(g); this.setForeground(Color.BLUE);
		 * 
		 * setFonts(g); String s1 = "caGrid 1.0 Installer";
		 * 
		 * int w1 = fm.stringWidth(s1);
		 * 
		 * 
		 * Dimension d = getSize(); int cx = (d.width - w1) / 2; int cy =
		 * (d.height - fm.getHeight()) / 2 + fm.getAscent();
		 * 
		 * g.setFont(f); g.drawString(s1, cx, cy);
		 */
	...
	private JLabel getBackgroundImage() {
		ImageScaler is = new ImageScaler();
		ImageIcon icon = new ImageIcon(is.getScaledInstance("background.jpg",
				600, 300));
