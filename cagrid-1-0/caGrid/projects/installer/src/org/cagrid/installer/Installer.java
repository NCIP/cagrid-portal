...
public class Installer {
	...
	private void initSteps(Map<String, String> defaultState) {
		...
		selectServicesStep.getOptions().add(
				new BooleanPropertyConfigurationOption(Constants.INSTALL_CDS,
						"Credential Delegation Service (CDS)", false, true));
		selectServicesStep.getOptions().add(
		...
		selectServicesStep.getOptions().add(
				new BooleanPropertyConfigurationOption(
						Constants.INSTALL_TRANSFER,
						"Transfer Service (Tomcat Only)", false, true));
		selectServicesStep.getOptions().add(
				new BooleanPropertyConfigurationOption(
		...
		// refacted to be
		// easier managed.
		PropertyConfigurationStep selectAllContainerStep = new PropertyConfigurationStep(
				this.model.getMessage("select.container.title"), this.model
						.getMessage("select.container.desc"));
		selectAllContainerStep
				.getOptions()
				.add(
						new ListPropertyConfigurationOption(
								...
								true));
		PropertyConfigurationStep selectTomcatOnlyContainerStep = new PropertyConfigurationStep(
				this.model.getMessage("select.container.title"), this.model
						.getMessage("select.container.desc"));
		selectTomcatOnlyContainerStep.getOptions().add(
				new ListPropertyConfigurationOption(Constants.CONTAINER_TYPE,
						this.model.getMessage("container.type"),
						new String[] { this.model
								.getMessage("container.type.tomcat"), }, true));
		...
		this.model.add(selectAllContainerStep, new Condition() {
			public boolean evaluate(WizardModel m) {
				...
				return model.isConfigureContainerSelected()
						&& !(model.isTrue(Constants.INSTALL_PORTAL) || model
								.isTrue(Constants.INSTALL_TRANSFER));
	...
	private void addConfigureContainerSteps() {
		...
		getHostnameStep.getOptions().add(
				new TextPropertyConfigurationOption(Constants.SERVICE_HOSTNAME,
						this.model.getMessage("service.hostname"), this.model
								.getProperty(Constants.SERVICE_HOSTNAME,
										"localhost"), true));
		...
		tomcatPortsStep.getOptions().add(
				new TextPropertyConfigurationOption(
						Constants.TOMCAT_SHUTDOWN_PORT, this.model
								.getMessage("tomcat.shutdown.port"), this.model
								.getProperty(Constants.TOMCAT_SHUTDOWN_PORT,
										"8005"), true));
		tomcatPortsStep.getOptions().add(
				new TextPropertyConfigurationOption(Constants.TOMCAT_HTTP_PORT,
						this.model.getMessage("tomcat.http.port"),
						this.model.getProperty(Constants.TOMCAT_HTTP_PORT,
								"8080"), true));
		tomcatPortsStep.getOptions().add(
				new TextPropertyConfigurationOption(
						Constants.TOMCAT_HTTPS_PORT, this.model
								.getMessage("tomcat.https.port"), this.model
								.getProperty(Constants.TOMCAT_HTTPS_PORT,
										"8443"), true));
		// TODO: add validation
