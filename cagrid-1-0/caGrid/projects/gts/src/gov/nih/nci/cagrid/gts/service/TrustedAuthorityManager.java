...
public class TrustedAuthorityManager {
		...
		try {
			...
			if (filter != null) {
				...
				if (filter.getName() != null) {
					...
					firstAppended = true;
					sql.append(" " + TrustedAuthorityTable.NAME + " = '"
							+ filter.getName() + "'");
				}
				...
				if (filter.getCertificateDN() != null) {
					sql = appendWhereOrAnd(firstAppended, sql);
					firstAppended = true;
					sql.append(" " + TrustedAuthorityTable.CERTIFICATE_DN
				...
				if (filter.getStatus() != null) {
					sql = appendWhereOrAnd(firstAppended, sql);
					firstAppended = true;
					sql.append(" " + TrustedAuthorityTable.STATUS + "='"
							+ filter.getStatus() + "'");
				}
				...
				if (filter.getTrustLevel() != null) {
					sql = appendWhereOrAnd(firstAppended, sql);
					firstAppended = true;
					sql.append(" " + TrustedAuthorityTable.TRUST_LEVEL + "='"
				...
				if (filter.getIsAuthority() != null) {
					sql = appendWhereOrAnd(firstAppended, sql);
					firstAppended = true;
					sql.append(" " + TrustedAuthorityTable.IS_AUTHORITY + "='"
				...
				if (filter.getAuthorityGTS() != null) {
					sql = appendWhereOrAnd(firstAppended, sql);
					firstAppended = true;
					sql.append(" " + TrustedAuthorityTable.AUTHORITY_GTS
				...
				if (filter.getSourceGTS() != null) {
					sql = appendWhereOrAnd(firstAppended, sql);
					firstAppended = true;
					sql.append(" " + TrustedAuthorityTable.SOURCE_GTS + " = '"
							+ filter.getSourceGTS() + "'");
				}
	...
	public synchronized void updateTrustedAuthority(TrustedAuthority ta,
		...
		if (internal) {
			...
			if (!ta.getAuthorityGTS().equals(curr.getAuthorityGTS())) {
				buildUpdate(needsUpdate, sql,
						TrustedAuthorityTable.AUTHORITY_GTS, ta
								.getAuthorityGTS());
				needsUpdate = true;
			}
			...
			if (ta.getCertificate() != null) {
				if ((clean(ta.getCertificate().getCertificateEncodedString()) != null)
					...
					buildUpdate(needsUpdate, sql,
							TrustedAuthorityTable.CERTIFICATE, ta
									.getCertificate()
			...
			if (!ta.getSourceGTS().equals(curr.getSourceGTS())) {
				buildUpdate(needsUpdate, sql, TrustedAuthorityTable.SOURCE_GTS,
						ta.getSourceGTS());
				needsUpdate = true;
			}

			if (ta.getExpires() != curr.getExpires()) {
				buildUpdate(needsUpdate, sql, TrustedAuthorityTable.EXPIRES, ta
						.getExpires());
				needsUpdate = true;
			}
		...
		if (ta.getCRL() != null) {
			if ((clean(ta.getCRL().getCrlEncodedString()) != null)
				...
				buildUpdate(needsUpdate, sql, TrustedAuthorityTable.CRL, ta
						.getCRL().getCrlEncodedString());
				needsUpdate = true;
			}
		}
		...
		if ((ta.getStatus() != null)
			...
			buildUpdate(needsUpdate, sql, TrustedAuthorityTable.STATUS, ta
					.getStatus().getValue());
			needsUpdate = true;
		}
		...
		if ((ta.getTrustLevel() != null)
			...
			buildUpdate(needsUpdate, sql, TrustedAuthorityTable.TRUST_LEVEL, ta
					.getTrustLevel());
			needsUpdate = true;
		}
		...
		try {
			if (!ta.equals(curr)) {
				if (needsUpdate) {
					...
					buildUpdate(needsUpdate, sql,
							TrustedAuthorityTable.LAST_UPDATED, ta
									.getLastUpdated());
					sql.append(" WHERE " + TrustedAuthorityTable.NAME + "='"
							+ ta.getName() + "'");
	...
	public synchronized TrustedAuthority getTrustedAuthority(String name)
		...
		String sql = "select * from " + TrustedAuthorityTable.TABLE_NAME
				+ " where " + TrustedAuthorityTable.NAME + "='" + name + "'";
		Connection c = null;
		try {
			c = db.getConnection();
			Statement s = c.createStatement();
	...
	public synchronized boolean doesTrustedAuthorityExist(String name)
		...
		this.buildDatabase();
		String sql = "select count(*) from " + TrustedAuthorityTable.TABLE_NAME
				+ " where " + TrustedAuthorityTable.NAME + "='" + name + "'";
		Connection c = null;
	...
	public synchronized void removeTrustedAuthority(String name)
		...
		if (doesTrustedAuthorityExist(name)) {
			String sql = "delete FROM " + TrustedAuthorityTable.TABLE_NAME
					+ " where " + TrustedAuthorityTable.NAME + "='" + name
					+ "'";
			try {
				db.update(sql);
	...
	public synchronized void removeTrustedAuthoritiesByLevel(String level)
		...
		buildDatabase();
		String sql = "delete FROM " + TrustedAuthorityTable.TABLE_NAME
				+ " where " + TrustedAuthorityTable.TRUST_LEVEL + "='" + level
				+ "'";
		try {
			db.update(sql);
		} catch (Exception e) {
	...
	private synchronized void insertTrustedAuthority(TrustedAuthority ta,
		...
		try {
			...
			insert.append("INSERT INTO " + TrustedAuthorityTable.TABLE_NAME
					...
					+ ta.getTrustLevel() + "', " + TrustedAuthorityTable.STATUS
					+ "='" + ta.getStatus().getValue() + "', "
					...
					+ ta.getSourceGTS() + "', " + TrustedAuthorityTable.EXPIRES
					+ "=" + ta.getExpires() + ", "
	...
	public void destroy() throws GTSInternalFault {
		try {
			buildDatabase();
			db.update("DROP TABLE IF EXISTS "
