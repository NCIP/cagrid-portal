...
public class TrustLevelManager {
	...
	public synchronized void addTrustLevel(TrustLevel level, boolean internal)
		...
		insert.append("INSERT INTO " + TrustLevelTable.TABLE_NAME + " SET "
				+ TrustLevelTable.NAME + "= ?," + TrustLevelTable.DESCRIPTION
	...
	public synchronized TrustLevel[] getTrustLevels(String gtsSourceURI)
		...
		try {
			...
			PreparedStatement s = c.prepareStatement("select * from "
					+ TrustLevelTable.TABLE_NAME + " WHERE "
					+ TrustLevelTable.SOURCE_GTS + "= ?");
	...
	public synchronized void updateTrustLevel(TrustLevel level, boolean internal)
		...
		if (internal) {
			...
			if (!curr.getAuthorityGTS().equals(level.getAuthorityGTS())) {
				update.addField(TrustLevelTable.AUTHORITY_GTS, level
						.getAuthorityGTS());
				needsUpdate = true;
			}

			if (!curr.getSourceGTS().equals(level.getSourceGTS())) {
				update.addField(TrustLevelTable.SOURCE_GTS, level
						.getSourceGTS());
				needsUpdate = true;
			}
		}
		...
		if (level.getDescription() != null) {
			if ((Utils.clean(level.getDescription()) != null)
				...
				update.addField(TrustLevelTable.DESCRIPTION, level
						.getDescription());
				needsUpdate = true;
			}
		}
		...
		try {
			if (!level.equals(curr)) {
				if (needsUpdate) {
					...
					Calendar cal = new GregorianCalendar();
					level.setLastUpdated(cal.getTimeInMillis());
					update.addField(TrustLevelTable.LAST_UPDATED, Long
							.valueOf(level.getLastUpdated()));
					update.addWhereField(TrustLevelTable.NAME, "=", level
							.getName());
