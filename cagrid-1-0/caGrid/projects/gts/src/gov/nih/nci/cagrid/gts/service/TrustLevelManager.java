...
public class TrustLevelManager {
	...
	public TrustLevelManager(String gtsURI,
			TrustedAuthorityLevelRemover status, DBManager dbManager) {
	...
	public synchronized void addTrustLevel(TrustLevel level)
			throws GTSInternalFault, IllegalTrustLevelFault {
		addTrustLevel(level, true);
	}

	public synchronized void addTrustLevel(TrustLevel level, boolean internal)
			throws GTSInternalFault, IllegalTrustLevelFault {
		this.buildDatabase();
		...
		if (internal) {
			...
			if ((level.getIsAuthority() == null)) {
				IllegalTrustLevelFault fault = new IllegalTrustLevelFault();
				fault
						.setFaultString("The trust level "
								+ level.getName()
			...
			if ((level.getIsAuthority().booleanValue())
					&& (!level.getAuthorityGTS().equals(gtsURI))) {
				IllegalTrustLevelFault fault = new IllegalTrustLevelFault();
				fault
						.setFaultString("The trust level "
								+ level.getName()
		...
		try {
			insert.append("INSERT INTO " + TrustLevelTable.TABLE_NAME + " SET "
					+ TrustLevelTable.NAME + "='" + level.getName() + "',"
					+ TrustLevelTable.DESCRIPTION + "='"
					+ level.getDescription() + "', "
					+ TrustLevelTable.IS_AUTHORITY + "='"
					+ level.getIsAuthority() + "', "
					+ TrustLevelTable.AUTHORITY_GTS + "='"
					+ level.getAuthorityGTS() + "', "
					+ TrustLevelTable.SOURCE_GTS + "='" + level.getSourceGTS()
			...
			this.log
					.error(
							"Unexpected database error incurred in adding the Trust Level, "
									+ level.getName()
									+ ", the following statement generated the error: \n"
									+ insert.toString() + "\n", e);
			GTSInternalFault fault = new GTSInternalFault();
			fault.setFaultString("Unexpected error adding the Trust Level, "
					+ level.getName() + "!!!");
			throw fault;
		}
	}
	...
	public synchronized TrustLevel[] getTrustLevels() throws GTSInternalFault {
		...
		try {
			...
			while (rs.next()) {
				...
				level.setIsAuthority(new Boolean(rs
						.getBoolean(TrustLevelTable.IS_AUTHORITY)));
				level.setAuthorityGTS(rs
						.getString(TrustLevelTable.AUTHORITY_GTS));
			...
			GTSInternalFault fault = new GTSInternalFault();
			fault
					.setFaultString("Unexpected error occurred in getting trust levels.");
			throw fault;
		} finally {
	...
	public synchronized TrustLevel[] getTrustLevels(String gtsSourceURI)
		...
		try {
			...
			sql.append("select * from " + TrustLevelTable.TABLE_NAME
					+ " WHERE " + TrustLevelTable.SOURCE_GTS + "='"
					+ gtsSourceURI + "'");
	...
	public synchronized TrustLevel getTrustLevel(String name)
			throws GTSInternalFault, InvalidTrustLevelFault {
		String sql = "select * from " + TrustLevelTable.TABLE_NAME + " where "
				+ TrustLevelTable.NAME + "='" + name + "'";
		Connection c = null;
		try {
			c = db.getConnection();
			Statement s = c.createStatement();
			...
			this.log
					.error(
							"Unexpected database error incurred in obtaining the trust level, "
									+ name
									+ ", the following statement generated the error: \n"
									+ sql + "\n", e);
			GTSInternalFault fault = new GTSInternalFault();
			fault.setFaultString("Unexpected error obtaining the trust level "
					+ name);
			throw fault;
		} finally {
			db.releaseConnection(c);
	...
	public synchronized void updateTrustLevel(TrustLevel level)
			throws GTSInternalFault, InvalidTrustLevelFault,
			IllegalTrustLevelFault {
		updateTrustLevel(level, true);
	}

	public synchronized void updateTrustLevel(TrustLevel level, boolean internal)
			throws GTSInternalFault, InvalidTrustLevelFault,
			IllegalTrustLevelFault {
		...
		if (internal) {
			if (!curr.getAuthorityGTS().equals(gtsURI)) {
				IllegalTrustLevelFault fault = new IllegalTrustLevelFault();
				fault
						.setFaultString("The trust level cannot be updated, the GTS ("
			...
			if ((Utils.clean(level.getAuthorityGTS()) != null)
				...
				IllegalTrustLevelFault fault = new IllegalTrustLevelFault();
				fault
						.setFaultString("The authority trust service for a trust level cannot be changed");
				throw fault;
			}
			...
			if ((Utils.clean(level.getSourceGTS()) != null)
				...
				IllegalTrustLevelFault fault = new IllegalTrustLevelFault();
				fault
						.setFaultString("The source trust service for a trust level cannot be changed");
				throw fault;
			}
			...
			if (!curr.getAuthorityGTS().equals(level.getAuthorityGTS())) {
				buildUpdate(needsUpdate, sql, TrustLevelTable.AUTHORITY_GTS,
						level.getAuthorityGTS());
				needsUpdate = true;
			}

			if (!curr.getSourceGTS().equals(level.getSourceGTS())) {
				buildUpdate(needsUpdate, sql, TrustLevelTable.SOURCE_GTS, level
						.getSourceGTS());
				needsUpdate = true;
			}
		}
		...
		if ((level.getIsAuthority() != null)
				&& (!level.getIsAuthority().equals(curr.getIsAuthority()))) {
			IllegalTrustLevelFault fault = new IllegalTrustLevelFault();
			fault
					.setFaultString("Whether or not this GTS is the authority for a level cannot be changed!!!");
			throw fault;
		}
		...
		if (level.getDescription() != null) {
			if ((Utils.clean(level.getDescription()) != null)
				...
				buildUpdate(needsUpdate, sql, TrustLevelTable.DESCRIPTION,
						level.getDescription());
				needsUpdate = true;
			}
		}
		...
		try {
			if (!level.equals(curr)) {
				if (needsUpdate) {
					Calendar c = new GregorianCalendar();
					level.setLastUpdated(c.getTimeInMillis());
					buildUpdate(needsUpdate, sql, TrustLevelTable.LAST_UPDATED,
							level.getLastUpdated());
					sql.append(" WHERE " + TrustLevelTable.NAME + "='"
							+ level.getName() + "'");
			...
			this.log
					.error(
							"Unexpected database error incurred in updating the trust level, "
									+ level.getName()
									+ ", the following statement generated the error: \n"
									+ sql.toString() + "\n", e);
			GTSInternalFault fault = new GTSInternalFault();
			fault
					.setFaultString("Unexpected error occurred in updating the trust level, "
							+ level.getName() + ".");
			throw fault;
		}
	}

	public synchronized void removeTrustLevel(String name)
			throws GTSInternalFault, InvalidTrustLevelFault,
			IllegalTrustLevelFault {
		if (doesTrustLevelExist(name)) {
			this.status.removeAssociatedTrustedAuthorities(name);
			String sql = "delete FROM " + TrustLevelTable.TABLE_NAME
					+ " where " + TrustLevelTable.NAME + "='" + name + "'";
			try {
				db.update(sql);
			} catch (Exception e) {
				this.log
						.error(
								"Unexpected database error incurred in removing the trust level, "
										+ name
										+ ", the following statement generated the error: \n"
										+ sql + "\n", e);
				GTSInternalFault fault = new GTSInternalFault();
				fault
						.setFaultString("Unexpected error removing the trust level "
								+ name);
				throw fault;
			}
		} else {
			InvalidTrustLevelFault fault = new InvalidTrustLevelFault();
			fault
					.setFaultString("The trust level " + name
							+ " does not exist.");
			throw fault;
		}
	}

	public synchronized boolean doesTrustLevelExist(String name)
			throws GTSInternalFault {
		this.buildDatabase();
		String sql = "select count(*) from " + TrustLevelTable.TABLE_NAME
				+ " where " + TrustLevelTable.NAME + "='" + name + "'";
		Connection c = null;
		...
		try {
			...
			this.log
					.error(
							"Unexpected database error incurred in determining if the TrustLevel "
									+ name
									+ " exists, the following statement generated the error: \n"
									+ sql + "\n", e);
			GTSInternalFault fault = new GTSInternalFault();
			fault
					.setFaultString("Unexpected error in determining if the Trust Level "
							+ name + " exists.");
			throw fault;
		} finally {
			db.releaseConnection(c);
		...
		if (!dbBuilt) {
			try {
				...
				if (!this.db.tableExists(TrustLevelTable.TABLE_NAME)) {
					String sql = dbManager.getTrustLevelTable()
							.getCreateTableSQL();
					db.update(sql);
				}
				dbBuilt = true;
