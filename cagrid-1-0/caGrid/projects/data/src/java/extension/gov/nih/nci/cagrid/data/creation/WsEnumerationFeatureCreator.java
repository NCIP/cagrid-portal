...
public class WsEnumerationFeatureCreator extends FeatureCreator {
	...
	public WsEnumerationFeatureCreator(ServiceInformation info,
			ServiceType mainService, Properties serviceProps) {
	...
	private void installWsEnumExtension() throws CreationExtensionException {
		...
		if (!wsEnumExtensionUsed()) {
			System.out
					.println("Adding the WS-Enumeration extension to the service");
			...
			System.arraycopy(serviceExtensions, 0, allExtensions, 0,
					serviceExtensions.length);
			allExtensions[allExtensions.length - 1] = extType;
			getServiceInformation().getServiceDescriptor().getExtensions()
					.setExtension(allExtensions);
	...
	private void addEnumerationQueryMethod() {
		...
		QName queryQname = new QName(DataServiceConstants.CQL_QUERY_URI,
				CQLQuery.class.getSimpleName());
		...
		enumOutput.setQName(new QName(WsEnumConstants.WS_ENUMERATION_URI,
				WsEnumConstants.ENUMERATE_RESPONSE_TYPE));
		enumerateMethod.setOutput(enumOutput);
		// import info
		MethodTypeImportInformation enumImport = new MethodTypeImportInformation();
		enumImport.setPortTypeName(EnumerationDataServicePortType.class
				.getSimpleName());
		enumImport.setWsdlFile("EnumerationDataService.wsdl");
		enumImport.setInputMessage(new QName(
				ENUMERATION_DATA_SERVICE_NAMESPACE, "EnumerationQueryRequest"));
		enumImport
				.setOutputMessage(new QName(ENUMERATION_DATA_SERVICE_NAMESPACE,
						"EnumerationQueryResponse"));
		enumImport.setNamespace(ENUMERATION_DATA_SERVICE_NAMESPACE);
		enumImport
				.setPackageName(DataServiceConstants.ENUMERATION_DATA_SERVICE_PACKAGE);
		...
		enumProvider.setProviderClass(EnumerationDataServiceProviderImpl.class
				.getName());
		...
		methodExceptions.setException(new MethodTypeExceptionsException[] {
	...
	private void copySchemas() throws CreationExtensionException {
		...
		File addressingXsdFile = new File(wsEnumExtensionSchemaDir
				.getAbsolutePath()
				+ File.separator + WsEnumConstants.ADDRESSING_XSD_NAME);
		File wsdlOutFile = new File(schemaDir + File.separator
				+ wsdlFile.getName());
		File enumWsdlOutFile = new File(schemaDir + File.separator
				+ enumWsdlFile.getName());
		File enumXsdOutFile = new File(schemaDir + File.separator
				+ enumXsdFile.getName());
		File addressingXsdOutFile = new File(schemaDir + File.separator
				+ addressingXsdFile.getName());
		try {
	...
	private void createDataSourceClient() throws CreationExtensionException {
		...
		int nameStart = clientClassContents.indexOf(classStart)
				+ classStart.length();
		int nameEnd = clientClassContents.indexOf(' ', nameStart);
		String clientClassName = clientClassContents.substring(nameStart,
				nameEnd);
		...
		String clientPackage = clientClassContents.substring(packNameStart,
				packNameEnd);
		// write it out to disk
		File clientClassFile = new File(getServiceInformation()
		...
		try {
			...
			throw new CreationExtensionException(
					"Error creating Data Source enumeration client class: "
							+ ex.getMessage(), ex);
		}
	}
	...
	private boolean wsEnumExtensionUsed() {
		ServiceDescription desc = getServiceInformation()
				.getServiceDescriptor();
		if ((desc.getExtensions() != null)
				&& (desc.getExtensions().getExtension() != null)) {
			for (int i = 0; i < desc.getExtensions().getExtension().length; i++) {
				if (desc.getExtensions().getExtension(i).getName().equals(
						WS_ENUM_EXTENSION_NAME)) {
					return true;
				}
			}
	...
	private String getServiceSchemaDir() {
		return getServiceProperties().getProperty(
