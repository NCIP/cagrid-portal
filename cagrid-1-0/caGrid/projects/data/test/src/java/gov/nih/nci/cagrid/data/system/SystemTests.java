 ...
 * @created Nov 7, 2006
 * @version $Id: SystemTests.java,v 1.10 2007-02-26 20:24:27 hastings Exp $
 */
public class SystemTests extends Story {
	public static final String INTRODUCE_DIR_PROPERTY = "introduce.base.dir";

	private static GlobusHelper globusHelper = new GlobusHelper(false,
			new File(IntroduceTestConstants.TEST_TEMP),
	...
	protected boolean storySetUp() {
		assertFalse("Globus should NOT be running yet", globusHelper
				.isGlobusRunning());
		return true;
	}
	...
	protected Vector steps() {
		...
		steps.add(new AddBookstoreStep(CreationTests.SERVICE_DIR,
				CreationTests.SERVICE_NAME));
		...
		steps.add(new RebuildServiceStep(getIntroduceBaseDir(),
				CreationTests.SERVICE_DIR, CreationTests.SERVICE_NAME,
				CreationTests.PACKAGE_NAME, CreationTests.SERVICE_NAMESPACE));
		// 5) set up a clean, temporary Globus
		steps.add(new CreateCleanGlobusStep(globusHelper));
		// 6) deploy data service
		steps.add(new DeployDataServiceStep(globusHelper,
				CreationTests.SERVICE_DIR));
		// 7) start globus
		steps.add(new StartGlobusStep(globusHelper));
		// 8) test data service
		steps.add(new InvokeDataServiceStep("localhost",
				IntroduceTestConstants.TEST_PORT + 1,
				CreationTests.SERVICE_NAME));
