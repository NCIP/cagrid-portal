...
public class TestIdentityFederationManager extends TestCase {
	...
	public void testRequestHostCertificateManualApproval() {
		...
		try {
			...
			ifs = new IdentityFederationManager(conf, db, props, ca,
	...
	public void testGetCRL() {
		...
		try {
			...
			for (int i = 0; i < totalUsers; i++) {
				...
				X509Certificate cert = ifs.requestUserCertificate(
						getSAMLAssertion(uid, idp2), publicKey, lifetime);
				String expectedIdentity = UserManager
						.subjectToIdentity(UserManager.getUserSubject(ifs
			...
			for (int i = 0; i < list.size(); i++) {
				...
				f.setDateRange(now);
				List<UserCertificateRecord> records = ifs
						.findUserCertificateRecords(adminGridId, f);
				for (int j = 0; j < records.size(); j++) {
					X509Certificate cert = CertUtil.loadCertificate(records
							.get(j).getCertificate().getCertificateAsString());
			...
			alreadyRevokedCertificates = this.validateCRLOnDisabledHostStatus(
					ifs, list, HostCertificateStatus.Suspended, adminGridId,
					userHostCerts, alreadyRevokedCertificates);
			alreadyRevokedCertificates = this.validateCRLOnDisabledHostStatus(
					ifs, list, HostCertificateStatus.Compromised, adminGridId,
					userHostCerts, alreadyRevokedCertificates);
			...
			for (int i = 0; i < list.size(); i++) {
				...
				CertificateLifetime lifetime = getLifetime();
				X509Certificate cert = ifs.requestUserCertificate(
						getSAMLAssertion(usr.getUser().getUID(), idp2),
						publicKey, lifetime);
				String expectedIdentity = UserManager
						.subjectToIdentity(UserManager.getUserSubject(ifs
								.getIdentityAssignmentPolicy(), ca
				...
				crl = ifs.getCRL();
				assertEquals(alreadyRevokedCertificates, crl
				...
				for (int j = 0; j < records.size(); j++) {
					X509Certificate cert2 = CertUtil.loadCertificate(records
							.get(j).getCertificate().getCertificateAsString());
					if (records.get(j).getSerialNumber() == cert
			...
			for (int i = 0; i < list.size(); i++) {
				...
				crl = ifs.getCRL();
				List<X509Certificate> compromisedUserCerts = usr
						.getCompromisedUserCerts();
				for (int j = 0; j < compromisedUserCerts.size(); j++) {
					assertNotNull(crl
							.getRevokedCertificate(compromisedUserCerts.get(j)));
				}
				...
				List<HostCertificateRecord> hostCerts = usr
						.getHostCertificates();
				for (int j = 0; j < compromisedUserCerts.size(); j++) {
					X509Certificate hostCert = CertUtil
							.loadCertificate(hostCerts.get(j).getCertificate()
									.getCertificateAsString());
	...
	public void testRequestCertificate() {
		...
		try {
			...
			CertificateLifetime lifetime = getLifetime();
			String uid = "user";
			X509Certificate cert = ifs.requestUserCertificate(getSAMLAssertion(
					uid, idp), publicKey, lifetime);
			String expectedIdentity = UserManager.subjectToIdentity(UserManager
					.getUserSubject(ifs.getIdentityAssignmentPolicy(), ca
	...
	public void testRequestCertificateSuspendedIdP() {
		...
		try {
			...
			CertificateLifetime lifetime = getLifetime();
			try {
				ifs.requestUserCertificate(getSAMLAssertion("user", idp),
						publicKey, lifetime);
	...
	public void testRequestCertificateManualApproval() {
		...
		try {
			...
			try {
				ifs.requestUserCertificate(getSAMLAssertion(username, idp),
	...
	public void testRequestCertificateInvalidAuthenticationMethod() {
		...
		try {
			...
			try {
				KeyPair pair = KeyUtil.generateRSAKeyPair1024();
				PublicKey publicKey = pair.getPublic();
				ifs.requestUserCertificate(getSAMLAssertionUnspecifiedMethod(
						username, idp), publicKey, getLifetime());
	...
	public void testRequestCertificateUntrustedIdP() {
		...
		try {
			...
			try {
				KeyPair pair = KeyUtil.generateRSAKeyPair1024();
				PublicKey publicKey = pair.getPublic();
				ifs.requestUserCertificate(getSAMLAssertion(username, idp2),
						publicKey, getLifetime());
	...
	public void testRequestCertificateExpiredAssertion() {
		...
		try {
			...
			try {
				KeyPair pair = KeyUtil.generateRSAKeyPair1024();
				PublicKey publicKey = pair.getPublic();
				ifs.requestUserCertificate(getExpiredSAMLAssertion(username,
						idp), publicKey, getLifetime());
	...
	private int validateAccessControl(IdentityFederationManager ifs,
			String adminId, String userId, int startCount) throws Exception {
	...
	private void validateCRLOnDisabledUserStatus(IdentityFederationManager ifs,
			List<UserContainer> list, GridUserStatus status,
	...
	private int validateCRLOnDisabledHostStatus(IdentityFederationManager ifs,
	...
	private GridUser createUser(IdentityFederationManager ifs,
		...
		PublicKey publicKey = pair.getPublic();
		CertificateLifetime lifetime = getLifetime();
		X509Certificate cert = ifs.requestUserCertificate(getSAMLAssertion(uid,
	...
	private FederationAuditFilter getHostCertificatedApprovedAuditingFilter(
			String target, String reporter) {
	...
	private void checkCertificate(String expectedIdentity,
			CertificateLifetime lifetime, PrivateKey key, X509Certificate cert)
			throws Exception {
		assertNotNull(cert);
		GlobusCredential cred = new GlobusCredential(key,
		...
		assertEquals(cert.getSubjectDN().toString(), identityToSubject(cred
				.getIdentity()));
		assertEquals(cred.getIssuer(), ca.getCACertificate().getSubjectDN()
				.getName());
		cred.verify();
	}
