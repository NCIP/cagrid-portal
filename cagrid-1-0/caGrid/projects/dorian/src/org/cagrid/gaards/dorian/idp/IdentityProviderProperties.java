...
public class IdentityProviderProperties {
	...
	public void setName(String name) throws DorianInternalFault {
		if (name.length() > MAX_NAME_LENGTH) {
			...
			f.setFaultString("The name of the Dorian IdP cannot exceed "
	...
	public void setAssertingCredentialsEncryptionPassword(
		...
		if (Utils.clean(assertingCredentialsEncryptionPassword) == null) {
			DorianInternalFault f = new DorianInternalFault();
			f
					.setFaultString("Invalid asserting credentials password specified.");
			throw f;
		}
	...
	public void setMinUserIdLength(int minUserIdLength)
		...
		if (minUserIdLength < MIN_UID_LENGTH) {
			DorianInternalFault f = new DorianInternalFault();
			f.setFaultString("The minimum user id must be at least "
					+ MIN_UID_LENGTH + " characters.");
			throw f;
		}
	...
	public void setMaxUserIdLength(int maxUserIdLength)
		...
		if (maxUserIdLength > MAX_UID_LENGTH) {
			DorianInternalFault f = new DorianInternalFault();
			f.setFaultString("The maximum user id must be no more than "
