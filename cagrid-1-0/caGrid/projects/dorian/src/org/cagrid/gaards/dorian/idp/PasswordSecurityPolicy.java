...
public class PasswordSecurityPolicy {
	...
	public void setConsecutiveInvalidLogins(int consecutiveInvalidLogins)
		...
		if (consecutiveInvalidLogins < MIN_CONSECUTIVE_INVALID_LOGINS) {
			...
			f
					.setFaultString("The number of invalid consecutive logins must be at least "
	...
	public void setTotalInvalidLogins(int totalInvalidLogins)
		...
		if (totalInvalidLogins < MIN_TOTAL_INVALID_LOGINS) {
			DorianInternalFault f = new DorianInternalFault();
			f
					.setFaultString("The number of total invalid logins must be at least "
	...
	public void setMinPasswordLength(int minPasswordLength)
		...
		if (minPasswordLength < MIN_PASSWORD_LENGTH) {
			DorianInternalFault f = new DorianInternalFault();
			f.setFaultString("The mininum password length must be at least "
					+ MIN_PASSWORD_LENGTH + ".");
			throw f;
		}
	...
	public void setMaxPasswordLength(int maxPasswordLength)
		...
		if (maxPasswordLength > MAX_PASSWORD_LENGTH) {
			DorianInternalFault f = new DorianInternalFault();
			f
					.setFaultString("The maximum password length must be no more than "
							+ MAX_PASSWORD_LENGTH + ".");
			throw f;
		}
