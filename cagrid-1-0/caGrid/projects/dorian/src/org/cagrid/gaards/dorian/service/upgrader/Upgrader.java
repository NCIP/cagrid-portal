...
public class Upgrader {
			...
			if (upgrades.size() == 0) {
				...
				for (int i = 0; i < upgrades.size(); i++) {
					...
					if (!properties.getVersion().equals(u.getStartingVersion())) {
						if (!trialRun) {
							throw new Exception("Cannot run the upgrader "
									+ u.getClass().getName()
									+ ", this upgrader starts with "
									+ u.getStartingVersion()
	...
	private List<Upgrade> determineUpgrades(String version,
	...
	public static void main(String[] args) {
		...
		Option conf = new Option(DORIAN_CONFIG_FILE_OPT,
				DORIAN_CONFIG_FILE_FULL, true,
		...
		Option trial = new Option(TRIAL_OPT, TRIAL_OPT_FULL, false,
				"Trial run of the upgrader.");
		options.addOption(trial);

		Option props = new Option(DORIAN_PROPERTIES_FILE_OPT,
				DORIAN_PROPERTIES_FILE_FULL, true,
		...
		try {
			...
			if (line.getOptionValue(HELP_OPT) != null) {
				...
				String dorianConfigFile = line
						.getOptionValue(DORIAN_CONFIG_FILE_OPT);
				String dorianPropertiesFile = line
						.getOptionValue(DORIAN_PROPERTIES_FILE_OPT);
				String upgraderConfigFile = line
						.getOptionValue(UPGRADER_CONFIG_FILE_OPT);

				BeanUtils beanUtils = new BeanUtils(new FileSystemResource(
						dorianConfigFile), new FileSystemResource(
						dorianPropertiesFile));
				UpgradeBeanUtils upgradeUtils = new UpgradeBeanUtils(
						new FileSystemResource(upgraderConfigFile));

				boolean trialRun = false;
				if (line.hasOption(TRIAL_OPT)) {
					trialRun = true;
				}
				Upgrader u = new Upgrader(beanUtils, upgradeUtils
						.getUpgradeList());
				u.upgrade(trialRun);
			}
