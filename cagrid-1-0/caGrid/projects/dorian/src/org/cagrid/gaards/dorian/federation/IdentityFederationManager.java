...
public class IdentityFederationManager extends LoggingObject implements Publisher {
    ...
    private void initializeEventManager() throws DorianInternalFault {
        try {
            ...
            this.eventManager.registerEventWithHandler(new EventToHandlerMapping(FederationAuditing.AccountRemoved
                .getValue(), AuditingConstants.GRID_ACCOUNT_AUDITOR));
            this.eventManager
                .registerEventWithHandler(new EventToHandlerMapping(FederationAuditing.SuccessfulUserCertificateRequest
                    .getValue(), AuditingConstants.GRID_ACCOUNT_AUDITOR));
            this.eventManager.registerEventWithHandler(new EventToHandlerMapping(
                FederationAuditing.InvalidUserCertificateRequest.getValue(), AuditingConstants.GRID_ACCOUNT_AUDITOR));
            this.eventManager.registerEventWithHandler(new EventToHandlerMapping(
                FederationAuditing.HostCertificateRequested.getValue(), AuditingConstants.HOST_AUDITOR));
            this.eventManager.registerEventWithHandler(new EventToHandlerMapping(
                FederationAuditing.HostCertificateApproved.getValue(), AuditingConstants.HOST_AUDITOR));
            this.eventManager.registerEventWithHandler(new EventToHandlerMapping(
                FederationAuditing.HostCertificateUpdated.getValue(), AuditingConstants.HOST_AUDITOR));
            this.eventManager.registerEventWithHandler(new EventToHandlerMapping(
                FederationAuditing.HostCertificateRenewed.getValue(), AuditingConstants.HOST_AUDITOR));

            this.eventManager.registerEventWithHandler(new EventToHandlerMapping(
                FederationAuditing.UserCertificateSigned.getValue(), AuditingConstants.USER_CERTIFICATE_AUDITOR));
            this.eventManager.registerEventWithHandler(new EventToHandlerMapping(
                FederationAuditing.UserCertificateUpdated.getValue(), AuditingConstants.USER_CERTIFICATE_AUDITOR));
            this.eventManager.registerEventWithHandler(new EventToHandlerMapping(
                FederationAuditing.UserCertificateRemoved.getValue(), AuditingConstants.USER_CERTIFICATE_AUDITOR));
        } catch (Exception e) {
    ...
    public void removeUserByLocalIdIfExists(X509Certificate idpCert, String localId) throws DorianInternalFault {
        try {
            ...
            removeUser(usr);
            this.eventManager.logEvent(usr.getGridId(), AuditConstants.SYSTEM_ID, FederationAuditing.AccountRemoved
    ...
    private void removeUser(GridUser usr) throws DorianInternalFault, InvalidUserFault {
        try {
            ...
            for (int i = 0; i < records.size(); i++) {
                ...
                if ((r.getStatus().equals(HostCertificateStatus.Active))
                    ...
                    this.eventManager.logEvent(String.valueOf(r.getId()), AuditConstants.SYSTEM_ID,
                        FederationAuditing.HostCertificateUpdated.getValue(),
            ...
            if (this.administrators.isMember(usr.getGridId())) {
                ...
                this.eventManager.logEvent(usr.getGridId(), AuditConstants.SYSTEM_ID, FederationAuditing.AdminRemoved
    ...
    public void addAdmin(String callerGridIdentity, String gridIdentity) throws RemoteException, DorianInternalFault,
        ...
        try {
            if (!this.administrators.isMember(gridIdentity)) {
                ...
                this.eventManager
                    .logEvent(gridIdentity, callerGridIdentity, FederationAuditing.AdminAdded.getValue(), "The user "
                        + gridIdentity + " was granted administrator privileges by " + callerGridIdentity + ".");
            }
        } catch (GroupException e) {
            logError(e.getMessage(), e);
            DorianInternalFault fault = new DorianInternalFault();
    ...
    public X509Certificate requestUserCertificate(SAMLAssertion saml, PublicKey publicKey, CertificateLifetime lifetime)
        ...
        if ((now.before(saml.getNotBefore())) || (now.after(saml.getNotOnOrAfter()))) {
            ...
            this.eventManager.logEvent(gid, AuditConstants.SYSTEM_ID, FederationAuditing.InvalidUserCertificateRequest
                .getValue(), msg);
        ...
        if (!um.determineIfUserExists(idp.getId(), uid)) {
            try {
                ...
                this.eventManager.logEvent(gid, AuditConstants.SYSTEM_ID, FederationAuditing.AccountCreated.getValue(),
        ...
        try {
            ...
            this.eventManager.logEvent(String.valueOf(userCert.getSerialNumber()), AuditConstants.SYSTEM_ID,
    ...
    public void removeUserCertificate(String callerIdentity, long serialNumber) throws DorianInternalFault,
        ...
        this.eventManager.logEvent(String.valueOf(serialNumber), callerIdentity,
            FederationAuditing.UserCertificateRemoved.getValue(), "User certificate (" + serialNumber + ") removed by "
                + callerIdentity + ".");
    }
