	...
	public static void main(String[] args) {
		...
		try {
			...
			if (line.getOptionValue(HELP_OPT) != null) {
				...
				formatter.printHelp(CreateCACertificate.class.getName(),
						options);
				System.exit(0);
			} else {
				...
				if (interactive) {
					...
					int count = 1;
					while (true) {
						ou = IOUtils.readLine("Enter Organizational Unit (OU) "
								+ count);
						count++;
					...
					String cn = IOUtils
							.readLine("Enter Common Name (CN)", true);
					sb.append(",CN=" + cn);
					sub = sb.toString();
					days = IOUtils.readInteger("Enter number of days valid",
							true);
				} else {
					...
					if (str == null) {
						...
						try {
							...
							throw new Exception(
									"The number of days specified was not a valid integer.");
						}
					}
