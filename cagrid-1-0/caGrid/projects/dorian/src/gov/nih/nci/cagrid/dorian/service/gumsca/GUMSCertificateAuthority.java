 ...
 * @version $Id: GUMSCertificateAuthority.java,v 1.1 2005-10-10 19:13:17 langella Exp $
 */
public class GUMSCertificateAuthority extends GUMSObject implements
	...
	private PrivateKey getCAPrivateKey() throws CertificateAuthorityFault,
		...
		try {
			...
			ResultSet rs = s.executeQuery("select PRIVATE_KEY from " + CA_TABLE
					+ " where ID='" + CA_USER + "'");
			if (rs.next()) {
	...
	private java.security.cert.X509Certificate getCACertificate(
		...
		try {
			c = db.getConnectionManager().getConnection();
			Statement s = c.createStatement();
			ResultSet rs = s.executeQuery("select CERTIFICATE from " + CA_TABLE
					+ " where ID='" + CA_USER + "'");
			if (rs.next()) {
	...
	public X509Certificate requestCertificate(
		...
		try {
			return CertUtil.signCertificateRequest(request, new X509Name(cacert
					.getSubjectDN().getName()), startDate, expirationDate,
					cakey);
		} catch (Exception e) {
			logError(e.getMessage(), e);
			CertificateAuthorityFault fault = new CertificateAuthorityFault();
