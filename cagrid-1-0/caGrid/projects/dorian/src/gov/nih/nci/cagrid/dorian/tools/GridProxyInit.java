package gov.nih.nci.cagrid.dorian.tools;
...
public class GridProxyInit {
	...
	public static void main(String[] args) {
		...
		Option authURL = new Option(AUTHENTICATION_SERVICE_URL,
				AUTHENTICATION_SERVICE_URL_VERBOSE, true,
				AUTHENTICATION_SERVICE_URL_DESCRIPTION);
		authURL.setRequired(true);
		options.addOption(authURL);

		Option dorianURL = new Option(DORIAN_URL, DORIAN_URL_VERBOSE, true,
				DORIAN_URL_DESCRIPTION);
		dorianURL.setRequired(true);
		options.addOption(dorianURL);

		Option userId = new Option(USER_ID, USER_ID_VERBOSE, true,
				USER_ID_DESCRIPTION);
		userId.setRequired(true);
		options.addOption(userId);

		Option password = new Option(PASSWORD, PASSWORD_VERBOSE, true,
				PASSWORD_DESCRIPTION);
		password.setRequired(true);
		options.addOption(password);

		Option ohours = new Option(LIFETIME_HOURS, LIFETIME_HOURS_VERBOSE,
				true, LIFETIME_HOURS_DESCRIPTION);
		ohours.setRequired(false);
		options.addOption(ohours);

		Option ominutes = new Option(LIFETIME_MINUTES,
				LIFETIME_MINUTES_VERBOSE, true, LIFETIME_MINUTES_DESCRIPTION);
		ominutes.setRequired(false);
		options.addOption(ominutes);

		Option oseconds = new Option(LIFETIME_SECONDS,
				LIFETIME_SECONDS_VERBOSE, true, LIFETIME_SECONDS_DESCRIPTION);
		...
		try {
			...
			if (ohours.getValue() != null) {
				try {
					...
					throw new Exception(
							"The number of hours must be specified as an integer!!!");
				}
			}
			...
			if (ominutes.getValue() != null) {
				try {
					...
					throw new Exception(
							"The number of minutes must be specified as an integer!!!");
				}
			}
			...
			if (oseconds.getValue() != null) {
				try {
					...
					throw new Exception(
							"The number of seconds must be specified as an integer!!!");
				}
			}
			...
			AuthenticationClient client = new AuthenticationClient(authURL
					.getValue(), cred);
			SAMLAssertion saml = client.authenticate();
