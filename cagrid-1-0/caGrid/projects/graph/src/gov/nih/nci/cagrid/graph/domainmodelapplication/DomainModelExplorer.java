...
public class DomainModelExplorer extends JLayeredPane {
	...
	public void initializeUMLDiagram(UMLDiagram d, DomainModel m, String name) {
		...
		// add classes
		for (int k = 0; k < this.model.getExposedUMLClassCollection().length; k++) {
			gov.nih.nci.cagrid.metadata.common.UMLClass c = model.getExposedUMLClassCollection()[k];

			if (c.getPackageName().equals(name)) {
				gov.nih.nci.cagrid.graph.uml.UMLClass C = new gov.nih.nci.cagrid.graph.uml.UMLClass(DomainModelOutline
	...
	public void initModel(DomainModel model) {
		...
		// uses O(n^2) algorithm to make a list of classes in each package in
		// the
		// domain model... could be left for improvement later.

		for (int k = 0; k < model.getExposedUMLClassCollection().length; k++) {
			gov.nih.nci.cagrid.metadata.common.UMLClass c = model.getExposedUMLClassCollection()[k];
	...
	protected void resizeComponents() {
		if (this.hideOutlines) {
			...
			if (!this.resizedOnce) {
				...
				this.umlMDI.setBounds(DomainModelExplorer.preferredOutlinesWidth + this.splitter.getWidth(), 0, this
					.getWidth()
					- DomainModelExplorer.preferredOutlinesWidth - this.splitter.getWidth(), this.getHeight());
				...
				this.umlMDI.setBounds(outlineMDI.getWidth() + this.splitter.getWidth(), 0, this.getWidth()
					- outlineMDI.getWidth() - this.splitter.getWidth(), this.getHeight());
			}
		}
	}
	...
	protected void requestSplitterMove(int x) {
		// if *PROJECTED/anticipated* move violates bounds THEN disallow move,
		// otherwise grant
	...
	protected void splitterMoved() {
		...
		umlMDI.setBounds(splitter.getX() + splitter.getWidth(), 0, getWidth() - outlineMDI.getWidth()
			- splitter.getWidth(), getHeight());
