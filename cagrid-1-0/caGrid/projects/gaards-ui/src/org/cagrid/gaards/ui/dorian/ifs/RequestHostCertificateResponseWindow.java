...
public class RequestHostCertificateResponseWindow extends ApplicationComponent {
     ...
     */
    public RequestHostCertificateResponseWindow(HostCertificateRecord record, PrivateKey privateKey, File directory) {
        super();
    ...
    private void reportOut() {
        try {
            directory.mkdirs();
            File keyPath = new File(directory.getAbsolutePath() + File.separator + record.getHost() + "-key.pem");
            KeyUtil.writePrivateKey(this.privateKey, keyPath);
            String str = null;
            if (record.getStatus().equals(HostCertificateStatus.Active)) {
                File certPath = new File(directory.getAbsolutePath() + File.separator + record.getHost() + "-cert.pem");
                ...
                str = "The certificate request for the host " + record.getHost()
                    + " has been approved.  The host's certificate has been written to " + certPath.getAbsolutePath()
                    + ".  The host's private key has been written to " + keyPath.getAbsolutePath()
    ...
    private JPanel getResponsePanel() {
        if (responsePanel == null) {
            ...
            responsePanel.setBorder(BorderFactory.createTitledBorder(null, "Report",
                    ...
                    public void actionPerformed(java.awt.event.ActionEvent e) {
                        try {
                            ...
                            GridApplication.getContext().addApplicationComponent(
                                new CertificateInformationComponent(cert), 600, 425);
                        } catch (Exception ex) {
