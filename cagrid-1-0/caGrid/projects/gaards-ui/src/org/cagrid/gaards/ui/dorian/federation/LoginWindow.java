...
public class LoginWindow extends ApplicationComponent {
		...
		if (handle != null) {
			...
			if ((profiles == null) || (profiles.size() <= 0)) {
				this.credentialLayout
						.show(
								getLoginPanel(),
								profileToString(AuthenticationProfile.BASIC_AUTHENTICATION));
				this.currentCredentialPanel = this.credentialPanels
						.get(AuthenticationProfile.BASIC_AUTHENTICATION);
			} else {
				QName profile = profiles.iterator().next();
				this.credentialLayout.show(getLoginPanel(),
	...
	private JPanel getLoginPanel() {
		if (loginPanel == null) {
			...
			this.credentialPanels.put(
					AuthenticationProfile.BASIC_AUTHENTICATION, panel1);
			this.credentialPanels.put(AuthenticationProfile.ONE_TIME_PASSWORD,
					panel2);
			...
			loginPanel.add(panel2,
					profileToString(AuthenticationProfile.ONE_TIME_PASSWORD));
		}
		return loginPanel;
	}
	...
	private void populateAuthenticationServices() {
		try {
			...
			if (handle != null) {
				List<AuthenticationServiceHandle> providers = handle
	...
	private void authenticate() {
		synchronized (mutex) {
			if (isCreating) {
				ErrorDialog
						.showError("Already trying to login, please wait!!!");
				return;
			} else {
				isCreating = true;
			}
		...
		DorianHandle dorian = ((DorianServiceListComboBox) this
				.getDorianService()).getSelectedService();
