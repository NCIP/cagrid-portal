 ...
 *          Exp $
 */
public class HostCertificatesWindow extends ApplicationComponent implements HostCertificateLauncher {
    ...
    private JPanel getContentPanel() {
        if (contentPanel == null) {
            ...
            contentPanel.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Host Certificates",
            ...
            viewHostCertificate.addActionListener(new java.awt.event.ActionListener() {
                public void actionPerformed(java.awt.event.ActionEvent e) {
                    Runner runner = new Runner() {
                        public void execute() {
                            try {
                                viewHostCertificate(getHostCertificatesTable().getSelectedHostCertificate());
                            } catch (Exception ex) {
        ...
        try {
            ...
            if (Utils.clean(getRecordId().getText()) != null) {
                try {
                    ...
                    throw new Exception("The record id must be an integer value.");
                }
            }
            ...
            if (Utils.clean(getSerialNumber().getText()) != null) {
                try {
                    filter.setSerialNumber(new BigInteger(getSerialNumber().getText()));
                } catch (NumberFormatException nfe) {
                    throw new Exception("The serial number must be an integer value.");
                }
            }
            ...
            if (getExpiration().getSelectedItem() instanceof Boolean) {
                filter.setIsExpired((Boolean) getExpiration().getSelectedItem());
            }
            ...
            if (certs != null) {
                ...
                for (int i = 0; i < certs.length; i++) {
                    this.getHostCertificatesTable().addHostCertificate(certs[i]);
                }
            }
            this.updateProgress(false, "Completed [Found " + length + " host certificates]");
    ...
    private JPanel getCriteriaPanel() {
        if (criteriaPanel == null) {
            ...
            criteriaPanel.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Search Criteria",
                javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
                javax.swing.border.TitledBorder.DEFAULT_POSITION, null, LookAndFeel.getPanelLabelColor()));
