 ...
 *          Exp $
 */
public class MyHostCertificatesWindow extends ApplicationComponent implements HostCertificateLauncher {
    ...
    public void viewHostCertificate(HostCertificateRecord record) {
        try {
            HostCertificateWindow window = new HostCertificateWindow(getSession().getServiceURI(), getSession()
                .getCredential(), getHostCertificatesTable().getSelectedHostCertificate(), false);
            GridApplication.getContext().addApplicationComponent(window, 600, 500);
        } catch (Exception e) {
    ...
    private void findHostCertificates() {
        ...
        try {
            ...
            IFSUserClient client = getSession().getUserClientWithCredentials();
            HostCertificateRecord[] certs = client.getOwnedHostCertificates();
            int length = 0;
            if (certs != null) {
                length = certs.length;
