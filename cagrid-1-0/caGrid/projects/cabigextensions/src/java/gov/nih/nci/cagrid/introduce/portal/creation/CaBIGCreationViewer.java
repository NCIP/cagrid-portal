...
public class CaBIGCreationViewer extends CreationViewerBaseComponent {
	...
	private void initialize() {
		this.setContentPane(new IconFeedbackPanel(validationModel,
				getMainPanel()));
	...
	private void initValidation() {
		...
		ValidationComponentUtils.setMessageKey(getServicePackage(),
				SERVICE_PACKAGE);
		ValidationComponentUtils.setMessageKey(getNamespaceDomain(),
				SERVICE_NAMESPACE);
	...
	private void validateInput() {
		...
		if (!ValidationUtils.isNotBlank(this.getDir().getText())) {
			result.add(new SimpleValidationMessage(SERVICE_DIR
					+ " must not be blank.", Severity.ERROR, SERVICE_DIR));
		} else {
		...
		} else if (!CommonTools.isValidPackageName(this.getServicePackage()
				.getText())) {
			result
					.add(new SimpleValidationMessage(
							SERVICE_PACKAGE
									+ " is not valid.  Service package must be in valid java package format. ("
									+ CommonTools.ALLOWED_JAVA_PACKAGE_REGEX
		...
		if (!ValidationUtils.isNotBlank(this.getNamespaceDomain().getText())) {
			...
			try {
				...
				result.add(new SimpleValidationMessage(SERVICE_NAMESPACE
						+ " is not a well formed namespace", Severity.ERROR,
						SERVICE_NAMESPACE));
			}
		}
	...
	private void addAnalyticalExtensions() {
		ServiceExtensionDescriptionType serviceExtension = ExtensionsLoader
				.getInstance().getServiceExtension(METADATA_EXTSION_NAME);
	...
	private void addDataExtensions() {
		ServiceExtensionDescriptionType serviceExtension = ExtensionsLoader
				.getInstance().getServiceExtension(DATA_EXTSION_NAME);
		String displayName = serviceExtension.getDisplayName();
		getExtensionsTable().addRow(displayName);
	}
	...
	private JPanel getInputPanel() {
		if (inputPanel == null) {
			...
			namespaceLabel
					.setText("STEP 4:  Enter a namespace for the generated WSDL:");
			...
			inputPanel.setBorder(javax.swing.BorderFactory.createTitledBorder(
					null, "Define the service",
					javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
					javax.swing.border.TitledBorder.DEFAULT_POSITION, null,
					PortalLookAndFeel.getPanelLabelColor()));
			packageLabel = new JLabel();
			packageLabel
					.setText("STEP 3:  Enter a Java package for the generated code:");
			...
			destinationLabel
					.setText("STEP 1:  Select a directory for your service:");
			...
			createButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					for (int i = 0; i < getExtensionsTable().getRowCount(); i++) {
						...
						try {
							edt = ExtensionsLoader.getInstance()
					...
					createService(
							getDir().getText(),
							getService().getText(),
							getServicePackage().getText(),
							getNamespaceDomain().getText(),
							Arrays
									.asList(new String[] {
					...
					try {
						ResourceManager.setStateProperty(
								ResourceManager.LAST_DIRECTORY, getDir()
										.getText());
					} catch (Exception e1) {
						e1.printStackTrace();
					}
		...
		if (addExtensionButton == null) {
			...
			addExtensionButton
					.addActionListener(new java.awt.event.ActionListener() {
						public void actionPerformed(java.awt.event.ActionEvent e) {
							if (!((String) getServiceStyleSeletor()
	...
	private JPanel getExtSelectionPanel() {
		if (extSelectionPanel == null) {
			...
			extSelectionPanel.add(getServiceStyleSeletor(),
					gridBagConstraints15);
			extSelectionPanel.add(getRemoveExtensionButton(),
					gridBagConstraints16);
			extSelectionPanel
					.add(getAddExtensionButton(), gridBagConstraints22);
		}
	...
	private JTabbedPane getJTabbedPane() {
		if (jTabbedPane == null) {
			...
			jTabbedPane.addTab("Standard", null, getServiceTyeSelectionPanel(),
					"Standard configuration options.");
			jTabbedPane
					.addTab("Advanced", null, getExtensionsPanel(),
	...
	private JPanel getServiceTyeSelectionPanel() {
		if (serviceTyeSelectionPanel == null) {
			...
			serviceTyeSelectionPanel.add(getDataRadioButton(),
					gridBagConstraints23);
			serviceTyeSelectionPanel.add(getAnalyticalRadioButton(),
