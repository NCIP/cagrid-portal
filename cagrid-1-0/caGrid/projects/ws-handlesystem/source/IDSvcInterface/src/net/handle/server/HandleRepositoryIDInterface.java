...
/**
 * A class providing a Handle System Repository backend for registering
 * resources into a global naming service.  This repository can be used
 * with a handle system; however a handle server is not created with this
 * class.  It also assumes that all handles will be stored locally on this
 * repository, so any attempts to create or access a resource stored on
 * another server will return a {@code null}.
 *
...
private String CREATE_HDL_STMT =
	"insert into handles ( prefix, handle, idx, type, data, ttl_type, ttl, "+
...
/**
 * Initializes the interface to the Handle System repository on the local
 * machine.
 *
...
public URI createOrGetGlobalID(ResourceIdInfo ID) {
	try {
		...
		if (needToCreate) {
			...
			AdminRecord adminRec = new AdminRecord(Util.encodeString(rootNA + "/" + serverNA), 200,
					true, true,
					false, false,
					true, true, true, true,
					true, true, true,
			...
			if (autoCommit) {
				storage.createHandle(Util.encodeString(fullContextHandle),
					new HandleValue[]{ contextURI,adminVal });
			} else {
				createHandle(Util.encodeString(fullContextHandle),
					new HandleValue[]{ contextURI,adminVal });
			}
		}
		...
		if (!needToCreate) {
			System.out.println("warning:  attribute " + fullAttributeHandle + " already created, no action performed");
		} else {
			...
			HandleValue adminVal = new HandleValue(100, Util.encodeString("HS_ADMIN"), Encoder.encodeAdminRecord(adminRec));
			if (autoCommit) {
				storage.createHandle(Util.encodeString(fullAttributeHandle),
					new HandleValue[]{ contextURI,adminVal });
			} else {
				createHandle(Util.encodeString(fullAttributeHandle),
					new HandleValue[]{ contextURI,adminVal });
			}
		}
		try {
...
public void removeGlobalID(URI ID) {
	...
	try {
		//System.out.println("removing " + new String(Util.encodeString(handleName)));
	...
	try {
		...
		if (!needToCreate) {
			...
			while (hdlsToDelete.hasMoreElements()) {
				...
				//System.out.println("deleting handle " + Util.decodeString(nextHandle));
				try {
			...
			storage.deleteHandle(Util.encodeString(fullContextHandle));
		}
	// for the exceptions, probably just do nothing.
	} catch (Exception e) {
		e.printStackTrace();
...
public void beginTransaction() throws HandleException {
	try {
		...
		System.err.println("Error, storage type " + storage.getClass().getName() + " cannot be used with transaction processing");
 ...
 * implementation, this method has no effect, other than displaying a message
...
private String nextHandleName(String prefix, String data) throws /*InvalidHandleFormatException,*/ HashCollisionException, HandleException {
	...
	for (int i=minHashLength; i<=longEncoded.length(); i++) {
		...
		HandleValue[] hv = getHandleFromStorage(handleName, new int[]{ 1 }, null);
...
private HandleValue[] getHandleFromStorage(String handle, int indexList[], byte typeList[][]) throws HandleException {
	// getRawHandleValues is transaction-safe, no need to switch
	byte[][] resp = storage.getRawHandleValues(Util.encodeString(handle),
			indexList, typeList);
	if (resp == null) {
		return null;
	}
...
private void createHandle(byte handle[], HandleValue values[]) throws /*HandleException,*/ SQLException {
	// no need to check if the handle already exists, we already did that in nextHandleName
	for(int i=0; i<values.length; i++) {
		...
		// if we really need this part later, we'll need to fix the undefined symbols.  For right now...
