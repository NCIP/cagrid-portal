...
/**
 * A class providing an API into a Handle System backend for registering
 * resources into a global naming service.  A Handle Server daemon is also
 * started on ports TCP/UDP 2641 and TCP 8000.
 *
 * <p>The {@link ResponseMessageCallback} interface, along with the
 * {@code handleResponse} method provided by the interface, is used
 * internally by the Handle System.</p>
 *
 * @see <a href="http://handle.net/documentation.html">Handle System Documentation</a>
 */
...
/**
 * Initializes the interface to the Handle System.  During initialization,
 * the Handle backend will prompt the user for the repository passphrase on
 * standard in.  The constructor also starts three listener threads for the
 * Handle protocol, one each on TCP/UDP port 2641 and TCP port 8000.
 *
 * <p>Because the Handle System also requires the administrator to create
 * a public/private key pair for authorization tasks like creating handles,
 * this constructor uses this key pair for registering the resources.
 * The key pair must already have been created during the Handle System
 * setup.</p>
 *
 * <p>The following properties must be defined in the <i>config.dct</i>
 * file:
 * <ul>
 * 	<li>{@code server_prefix}:  the handle prefix for the local server</li>
 * 	<li>{@code min_hash_length}:  the minimum size hash length for constructing URIs (between 1 and 32)</li>
 * 	<li>{@code auth_handle}:  the name of a handle containing a
 * 	public key with administrator privileges</li>
 * 	<li>{@code auth_index}:  the index of the public key within the
 * 	handle (usually 300)</li>
 * 	<li>{@code privKey_filename}:  the full path to the corresponding
 * 	private key</li>
 * 	<li>{@code privKey_passphrase}:  the passphrase protecting the
 * 	private key</li>
 * </ul>
 * </p>
 *
 * @param handleRepositoryDir
 * 	a {@link String} containing the absolute path to the Handle Repository
 * @throws Exception
 * 	If initialization of the repository fails.  Future versions of this
 * 	code may throw {@link HandleException} instead, for more automated
 * 	error-handling.
 */
public HandleServerIDInterface(String handleRepositoryDir) throws Exception {
	...
	main.initialize();
	main.start();   // this starts up the other listener threads-- TCP/UDP 2641 and TCP 8000
	...
	authInfo = new PublicKeyAuthenticationInfo(Util.encodeString(authHandle),
			authIndex, Util.getPrivateKeyFromBytes(keyData, 0));
}
...
public URI createOrGetGlobalID(ResourceIdInfo ID) {
	try {
		...
		String fullContextHandle = nextHandleName(serverNA, ID.resourceContext.toString());
		if (needToCreate) {
			HandleValue contextURI = new HandleValue(1, Util.encodeString("URI"), Util.encodeString(ID.resourceContext.toString()));
			AdminRecord adminRec = new AdminRecord(Util.encodeString(authHandle), authIndex,
					true, true,
					false, false,
					true, true, true, true,
					true, true, true,
					true);
			HandleValue adminVal = new HandleValue(100, Util.encodeString("HS_ADMIN"), Encoder.encodeAdminRecord(adminRec));
			CreateHandleRequest createContext = new CreateHandleRequest(Util.encodeString(fullContextHandle), new HandleValue[]{ contextURI,adminVal }, null);
			...
			//System.out.println("response (" + resp.getClass().getName() + "):  " + resp);
		}

		// make sure we register the sub-prefix with the server
		// (might not be necessary if sub-prefix delegation gets implemented)
		// convert prefix/subprefix handle into prefix.subprefix
		String subprefix = fullContextHandle.replace('/', '.');
		GenericRequest homeNAReq = new GenericRequest(Util.encodeString(rootNA + "/" + subprefix), AbstractMessage.OC_HOME_NA, null);
		...
		//System.out.println("response for homeNAReq:  (" + resp.getClass().getName() + ") " + resp);

		// create handle for resourceIdentification
		String fullAttributeHandle = nextHandleName(subprefix, ID.resourceIdentification);
		if (!needToCreate) {
			System.out.println("warning:  attribute already created, no action performed");
		} else {
			HandleValue contextURI = new HandleValue(1, Util.encodeString("Attribute"), Util.encodeString(ID.resourceIdentification));
			AdminRecord adminRec = new AdminRecord(Util.encodeString(authHandle), authIndex,
					true, true,
					false, false,
					true, true, true, true,
					...
					true);
			HandleValue adminVal = new HandleValue(100, Util.encodeString("HS_ADMIN"), Encoder.encodeAdminRecord(adminRec));
			CreateHandleRequest createContext = new CreateHandleRequest(Util.encodeString(fullAttributeHandle), new HandleValue[]{ contextURI,adminVal }, null);
			server.processRequest(createContext, this);
			if (resp instanceof ChallengeResponse) {
				handleChallenge((ChallengeResponse)resp, createContext);
			}
		...
		try {
			return new URI("hdl://" + fullAttributeHandle);
...
public ResourceIdInfo getBigIDInfo(URI identifier) {
	...
	System.out.println("\tscheme-specific part = " + identifier.getSchemeSpecificPart());
	System.out.println("\tuser info = " + identifier.getUserInfo());*/
	...
	try {
		...
		// separating the last part of the sub-prefix (replacing the last '.' with a '/')
		...
		ResolutionRequest lookup = new ResolutionRequest(Util.encodeString(contextHandle), null, new int[]{ 1 }, null);
...
public void removeGlobalID(ResourceIdInfo ID) {
	try {
		...
		String subprefix = fullContextHandle.replace('/', '.');
		String fullAttributeHandle = nextHandleName(subprefix, ID.resourceIdentification);
		removeGlobalID(new URI("hdl://" + fullAttributeHandle));
...
public void removeContext(URI context) {
	try {
		String fullContextHandle = nextHandleName(serverNA, context.toString());
		...
		if (!needToCreate) {
			...
			for (int i=0; i<hdlsToDelete.handles.length; i++) {
				//System.out.println("deleting handle " + Util.decodeString(hdlsToDelete.handles[i]));
			...
			//System.out.println("deleting context handle " + fullContextHandle);
			DeleteHandleRequest delContext = new DeleteHandleRequest(Util.encodeString(fullContextHandle), null);
...
/**
 * Browses the Handle System repository to find a hash-based handle
 * name for the given data under the given prefix.  Also signals via
 * the {@link needToCreate} variable whether this handle has not
 ...
 * @return
 * 	A hash-based string that can be used as a new handle name.
 ...
 * 	handle.
 */
private String nextHandleName(String prefix, String data) throws /*InvalidHandleFormatException,*/ HashCollisionException, HandleException {
	md.reset();
	...
	for (int i=minHashLength; i<=longEncoded.length(); i++) {
		String handleName = prefix + "/" + longEncoded.substring(0, i);
		// expecting to find the context URI or resource data at index 1
		ResolutionRequest lookup = new ResolutionRequest(Util.encodeString(handleName), null, new int[]{ 1 }, null);
		server.processRequest(lookup, this);
	...
	// we shouldn't ever get this far, otherwise we've found a SHA-1 collision
 ...
 * @throws HandleException
 * 	If the authentication process fails or if the server is unable
 * 	to process the challenge answer.
 */
private void handleChallenge(ChallengeResponse challenge, AbstractRequest req) throws HandleException {
	...
	ChallengeAnswerRequest reply = new ChallengeAnswerRequest(
			authInfo.getAuthType(), authInfo.getUserIdHandle(),
