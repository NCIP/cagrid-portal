<%--
Created by IntelliJ IDEA.
User: kherm
Date: Jul 6, 2005
Time: 2:47:35 PM
To change this template use File | Settings | File Templates.
--%>


<%@ page session="true" contentType="text/html;charset=utf-8" %>

<%@ taglib uri="http://java.sun.com/jsf/html" prefix="h" %>
<%@ taglib uri="http://java.sun.com/jsf/core" prefix="f" %>


<f:loadBundle basename="labels" var="labels"/>
<f:loadBundle basename="messages" var="messages"/>

<f:view locale="#{browserConfig.locale}">

<h:form>
<f:verbatim><br></f:verbatim>

<h:panelGrid width="100%" border="0" cellspacing="0" cellpadding="0">
    <h:column>
        <h:outputLink value="DiscoveryResults.tiles" styleClass="formText">
            <h:outputText value="#{messages.backToSearchResults}"></h:outputText>
        </h:outputLink>
    </h:column>
</h:panelGrid>

<f:verbatim><br></f:verbatim>


<h:panelGrid headerClass="formHeader" columns="1"
             width="100%" border="0" cellpadding="0" cellspacing="0">
    <f:facet name="header">
        <h:column>
            <h:outputText value="#{messages.appServiceDetailstitle}"/>
        </h:column>
    </f:facet>

    <!--Service Details-->
    <h:column>
        <h:panelGrid headerClass="formTitle"
                     columns="2" columnClasses="detailsCellKey,detailsCellValue"
                     width="100%" cellpadding="3" cellspacing="0">
            <f:facet name="header">
                <h:column>
                    <h:outputText value="#{messages.appServiceDetailstitle}"/>
                </h:column>
            </f:facet>

            <h:column>
                <h:outputText value="#{messages.appServiceName}"/>
            </h:column>
            <h:column>
                <h:outputText value="#{discoveryResult.navigatedService.name}"/>
            </h:column>

            <h:column>
                <h:outputText value="#{messages.appServiceVersion}"/>
            </h:column>
            <h:column>
                <h:outputText
                        value="#{discoveryResult.navigatedService.version}"/>
            </h:column>

        </h:panelGrid>
    </h:column>

    <!--RC Details-->
    <h:column>
        <h:panelGrid headerClass="formTitle"
                     columns="2" columnClasses="detailsCellKey,detailsCellValue"
                     width="100%" cellpadding="3" cellspacing="0">
            <f:facet name="header">
                <h:column>
                    <h:outputText value="#{messages.rcTitle}"/>
                </h:column>
            </f:facet>

            <h:column>
                <h:outputText value="#{messages.rcShortName}"/>
            </h:column>
            <h:column>
                <h:outputText value="#{discoveryResult.navigatedService.rcInfo.shortName}"/>
            </h:column>

            <h:column>
                <h:outputText value="#{messages.rcDisplayName}"/>
            </h:column>
            <h:column>
                <h:outputText
                        value="#{discoveryResult.navigatedService.rcInfo.displayName}"/>
            </h:column>

            <h:column>
                <h:outputText value="#{messages.rcAddress}"/>
            </h:column>
            <h:column>
                <h:outputText
                        value="#{discoveryResult.navigatedService.rcInfo.address.country}"/>
            </h:column>
        </h:panelGrid>
    </h:column>

</h:panelGrid>

<f:verbatim><br/></f:verbatim>

<!-- Service URL -->

<h:panelGrid headerClass="formHeader" columnClasses="formText"
             width="100%" border="1" cellpadding="3" cellspacing="0">
    <f:facet name="header">
        <h:column>
            <h:outputText value="#{messages.serviceURLtitle}"/>
        </h:column>
    </f:facet>


    <h:column>
        <h:commandLink
                action="#{discoveryResult.navigatedService.doSetNavigatedService}"
                value="#{discoveryResult.navigatedService.url}"/>
    </h:column>

</h:panelGrid>

<f:verbatim><br/></f:verbatim>

<%--cadsr information--%>
<h:panelGrid headerClass="formHeader" columns="1"
             width="100%" border="0" cellpadding="0" cellspacing="0"
        >
    <f:facet name="header">
        <h:column>
            <h:outputText value="#{messages.caDSRDetailsTitle}"/>
        </h:column>
    </f:facet>

    <!--caDSR Registration Details-->
    <h:column rendered="#{not empty discoveryResult.navigatedService.caDSRRegistration}">
        <h:panelGrid headerClass="formTitle"
                     columns="2" columnClasses="detailsCellKey,detailsCellValue"
                     width="100%" cellpadding="3" cellspacing="0">
            <f:facet name="header">
                <h:column>
                    <h:outputText value="#{messages.caDSRRegistrationTitle}"/>
                </h:column>
            </f:facet>

            <h:column>
                <h:outputText value="#{messages.caDSRregistrationStatus}"/>
            </h:column>
            <h:column>
                <h:outputText value="#{discoveryResult.navigatedService.caDSRRegistration.registrationStatus}"/>
            </h:column>

            <h:column>
                <h:outputText value="#{messages.caDSRregistrationWorkflowStatus}"/>
            </h:column>
            <h:column>
                <h:outputText
                        value="#{discoveryResult.navigatedService.caDSRRegistration.workflowStatus}"/>
            </h:column>

        </h:panelGrid>
    </h:column>

    <!--domain model info-->

    <h:column rendered="#{not empty discoveryResult.navigatedService.domainModel}">
        <h:dataTable var="umlclass"
                     value="#{discoveryResult.navigatedService.domainModel.exposedUMLAssociationCollection}"
                     headerClass="formTitle"
                     columnClasses="detailsCellKey,detailsCellValue"
                     width="100%" cellpadding="3" cellspacing="0">
            <f:facet name="header">
                <h:column>
                    <h:outputText value="#{messages.domainModelTitle}"/>
                </h:column>
            </f:facet>

            <h:column>
                <h:outputText value="#{umlclass.className}"/>
            </h:column>
            <h:column>
                <h:outputText value="#{umlclass.packageName}"/>

            </h:column>
        </h:dataTable>
    </h:column>

    <!--Operations Collection-->
    <h:column rendered="#{not empty discoveryResult.navigatedService.opersCollection}">
        <h:dataTable var="oper" value="#{discoveryResult.navigatedService.opersCollection}"
                     headerClass="formTitle"
                     columnClasses="detailsCellKey,detailsCellValue"
                     width="100%" cellpadding="3" cellspacing="0">
            <f:facet name="header">
                <h:column>
                    <h:outputText value="#{messages.operationTitle}"/>
                </h:column>
            </f:facet>


            <h:column>
                <h:outputText value="#{oper.name}"/>
            </h:column>
            <h:column>
                <h:outputText value="#{oper.description}"/>
            </h:column>
        </h:dataTable>
    </h:column>

</h:panelGrid>

<f:verbatim><br/></f:verbatim>


<h:panelGrid width="100%" border="0" cellpadding="3" cellspacing="0">
    <h:column>
        <h:commandButton id="addServiceCart" value="#{labels.addServiceCart}"
                         action="#{discoveryResult.navigatedService.doAddToServiceCart}">
        </h:commandButton>

    </h:column>
</h:panelGrid>

<f:verbatim><br/></f:verbatim>


</h:form>
</f:view>


