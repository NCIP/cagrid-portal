...
import gov.nih.nci.cagrid.cabio.client.CaBIOClient;
...
public class CaBIOClientTestCase extends TestCase {
    ...
    private CaBIOClient gridSvcClient;
	...
	try {
	    gridSvcClient = new CaBIOClient(gridSvcUrl);
	} catch (Exception ex) {
	    ex.printStackTrace();
	...
	try {
	    ...
	    qXRef.setGene(gene);
	    aXRefList = appSvcClient.search(DatabaseCrossReference.class, qXRef);
	} catch (Exception ex) {
	    ex.printStackTrace();
