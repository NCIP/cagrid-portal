...
import gov.nih.nci.cagrid.cabio.common.CaBIOSvcI;
import gov.nih.nci.cagrid.cabio.stubs.CaBIOSvcPortType;
import gov.nih.nci.cagrid.cabio.stubs.service.CaBIOSvcServiceAddressingLocator;
 ...
 */
public class CaBIOSvcClient extends ServiceSecurityClient implements CaBIOSvcI {
	protected CaBIOSvcPortType portType;

	private Object portTypeMutex;

	public CaBIOSvcClient(String url) throws MalformedURIException,
			RemoteException {
		this(url, null);
	}

	public CaBIOSvcClient(String url, GlobusCredential proxy)
			throws MalformedURIException, RemoteException {
		super(url, proxy);
		initialize();
	}

	public CaBIOSvcClient(EndpointReferenceType epr)
			throws MalformedURIException, RemoteException {
		this(epr, null);
	}

	public CaBIOSvcClient(EndpointReferenceType epr, GlobusCredential proxy)
			throws MalformedURIException, RemoteException {
		super(epr, proxy);
		initialize();
	}
	...
	private CaBIOSvcPortType createPortType() throws RemoteException {

		CaBIOSvcServiceAddressingLocator locator = new CaBIOSvcServiceAddressingLocator();
		...
		CaBIOSvcPortType port = null;
		try {
			port = locator.getCaBIOSvcPortTypePort(getEndpointReference());
		} catch (Exception e) {
	...
	public static void usage() {
		System.out.println(CaBIOSvcClient.class.getName()
				+ " -url <service url>");
	}
		...
		try {
			if (!(args.length < 2)) {
				if (args[0].equals("-url")) {
					CaBIOSvcClient client = new CaBIOSvcClient(args[1]);

					CQLQuery query = (CQLQuery) Utils.deserializeDocument(
							"test/resources/gene1.xml",
							CQLQuery.class);
