...
this.afterUpdateCallback(element, selectedElement,this.options.internalArray[this.index]);
},
...
var count     = 0;
var internalArray = new Array(0);
var internalPartialArray = new Array(0);
var partialCount = 0;
...
ret.push("
<li><strong>" + elem.substr(0, entry.length) + "</strong>" +
    elem.substr(entry.length) + "
</li>");
internalArray[count++] = instance.options.array[i];
break;
...
partial.push("
<li>" + elem.substr(0, foundPos) + "<strong>" +
    elem.substr(foundPos, entry.length) + "</strong>" +
    elem.substr(foundPos + entry.length) + "
</li>");
internalPartialArray[partialCount++] = instance.options.array[i];
break;
}
}
