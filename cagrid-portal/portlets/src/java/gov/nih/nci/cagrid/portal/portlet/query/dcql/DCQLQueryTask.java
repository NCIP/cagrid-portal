...
public class DCQLQueryTask implements Callable {
	...
	public Object call() throws Exception {
		...
		try {
			...
			FederatedQueryResultsClient resultsClilent = client
			...
			DCQLQueryResultsCollection dcqlResultsCol = resultsClilent
					.getResults();

			StringWriter writer = new StringWriter();
			Utils.serializeObject(dcqlResultsCol, new QName(
					"http://caGrid.caBIG/1.0/gov.nih.nci.cagrid.dcqlresult",
					"DCQLQueryResultsCollection"), writer);
			...
			logger.debug("Error running query: " + t.getMessage(), t);
			Exception ex = new Exception("Error running query: " + t.getMessage(), t);
			listener.onError(instance, ex);
			throw ex;
		}
		return out;
