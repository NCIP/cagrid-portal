...
import model2.service.Model2SvcConfiguration;
import java.io.File;
 ...
 * 
 */
public abstract class Model2SvcResourceBase extends ReflectionResource implements Resource
                                                  {

	static final Log logger = LogFactory.getLog(Model2SvcResourceBase.class);

	private Model2SvcResourceConfiguration configuration;
	...
	public gov.nih.nci.cagrid.metadata.ServiceMetadata getServiceMetadata(){
		return ((Model2SvcResourceProperties) getResourceBean()).getServiceMetadata();
	}
	
	public void setServiceMetadata(gov.nih.nci.cagrid.metadata.ServiceMetadata serviceMetadata ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(Model2SvcConstants.SERVICEMETADATA);
	...
	public gov.nih.nci.cagrid.metadata.dataservice.DomainModel getDomainModel(){
		return ((Model2SvcResourceProperties) getResourceBean()).getDomainModel();
	}
	
	public void setDomainModel(gov.nih.nci.cagrid.metadata.dataservice.DomainModel domainModel ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(Model2SvcConstants.DOMAINMODEL);
	...
	public Model2SvcResourceConfiguration getConfiguration() {
		...
		try {
			...
			this.configuration = (Model2SvcResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
	...
	private void loadServiceMetadataFromFile() {
      if(getServiceMetadata()==null){
		try {
			...
			((Model2SvcResourceProperties) this.getResourceBean()).setServiceMetadata((gov.nih.nci.cagrid.metadata.ServiceMetadata) Utils.deserializeDocument(dataFile.getAbsolutePath(),
	...
	private void loadDomainModelFromFile() {
      if(getDomainModel()==null){
		try {
			...
			((Model2SvcResourceProperties) this.getResourceBean()).setDomainModel((gov.nih.nci.cagrid.metadata.dataservice.DomainModel) Utils.deserializeDocument(dataFile.getAbsolutePath(),
